...
endpoint 可配置一到八个属于同一 PeerId 的 relay 传输地址；`yon-relay` 的 `listen` 与可被客户端拨号的 `external` 也都必须各提供一到八个地址。WSS 地址使用 `/tcp/<PORT>/tls/ws`；endpoint 配置 `wss_ca_der`，relay 配置 `wss_certificate_der` 和 `wss_private_key_der`。未知字段、非法文件、非 UTF-8、超过 64 KiB 的配置或无效组合都会使启动失败，不会静默降级。

只有 WSS 需要这组运维侧证书。自签证书可以使用：relay 配置带 `CA:FALSE`、`serverAuth` 和正确 SAN 的自签叶证书及私钥，两个 endpoint 把同一证书配置为 `wss_ca_der`。使用私有 CA 时，endpoint 改为信任该 CA。通过 IP 连接必须有对应 `IP SAN`，通过域名连接必须有对应 `DNS SAN`；只设置 `CN` 无效。relay 会在监听前解析证书/私钥 DER，并校验每个 WSS external 的 SAN；有效期、`CA`/`serverAuth` 用途、证书链、信任关系及证书与私钥的密码学匹配最终由客户端 TLS 握手验证，失败时关闭连接且绝不降级为明文。当前服务端只发送一个叶证书 DER，因此支持自签叶证书或根 CA 直接签发的叶证书，不支持需要发送 intermediate chain 的部署。
