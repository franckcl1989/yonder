...
mod tests {
    ...
    #[test]
    fn non_directory_storage_components_and_non_file_identity_are_rejected() {
        let dir = tempdir().unwrap();
        let root_file = dir.path().join("audit-file");
        fs::write(&root_file, b"not a directory").unwrap();
        assert!(matches!(
            open_or_create_identity(&root_file, &mut OsSecureRandom),
        ...
        fs::create_dir_all(&root).unwrap();
        SystemPrivateDirectoryPolicy.protect(&root).unwrap();
        fs::write(root.join(RECORDS_DIR_NAME), b"not a directory").unwrap();
        assert!(matches!(
            open_or_create_identity(&root, &mut OsSecureRandom),
            Err(AuditIdentityError::AuditIdentityPermissions)
        ));
        ...
        let root = test_root();
        ensure_audit_dirs(&root).unwrap();
        fs::create_dir(root.join(IDENTITY_FILE_NAME)).unwrap();
        assert!(matches!(
            open_or_create_identity(&root, &mut OsSecureRandom),
            Err(AuditIdentityError::AuditIdentityInvalid)
        ));
        ...
        assert!(matches!(
            ensure_audit_dirs(Path::new("invalid\0audit-root")),
            Err(AuditIdentityError::CreateDirectoryFailed(_))
        ));
    }

    #[test]
    fn exclusive_identity_creation_race_loads_the_winner() {
        let root = test_root();
        let winner = open_or_create_identity(&root, &mut OsSecureRandom).unwrap();
        let loaded =
            create_identity_file(&root.join(IDENTITY_FILE_NAME), &root, &mut OsSecureRandom)
                .unwrap();
        assert_eq!(loaded.public_key(), winner.public_key());
    }
