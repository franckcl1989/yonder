...
//! [`AuditWriter`] and the `/yonder/audit/2.0.0` substream halves behind a
...
const FILE_TRANSFER_ID_LABEL: &[u8] = b"yonder-audit-file-transfer-id-v2";
...
impl AuditObserver {
    ...
    pub async fn record_send_outcome(
        ...
        self.record(|session, now| session.record_send_outcome(direction, succeeded, bytes, now))
            .await
    }
    ...
    pub async fn record_pty_write_outcome(
        ...
        self.record(|session, now| session.record_pty_write_outcome(succeeded, bytes, now))
            .await
    }
    ...
    pub async fn record_display_write_outcome(
        ...
        self.record(|session, now| session.record_display_write_outcome(succeeded, bytes, now))
            .await
    }
    ...
    pub async fn record_terminal_exit(&self, exit_code: u8) -> Result<(), AuditError> {
    ...
    async fn record<'a>(
        ...
        let result = step(&mut core.session, self.now_ns());
        match result {
            Ok(batch) => core.writer.append_batch(batch).await,
            Err(error) => {
    ...
    pub async fn wait_for_frame(&self) -> Result<Option<Vec<u8>>, AuditError> {
        ...
        read_one_frame(read.as_mut()).await
    }
