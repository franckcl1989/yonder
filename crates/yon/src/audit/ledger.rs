...
fn recover_pending_records(
    ...
    let mut candidate: Option<LedgerCommit> = None;
    for entry in entries {
        ...
        if let Some(commit) = inspect_pending_record(&path, identity, head)?
            && candidate.replace(commit).is_some()
        {
            return Err(AuditLedgerError::AuditLedgerConflict);
        }
    }
...
mod tests {
    ...
    #[test]
    fn ledger_rejects_non_file_lock_and_state_components() {
        let root = test_root();
        drop(Ledger::open(&root, &mut OsSecureRandom).unwrap());
    ...
    #[test]
    fn recovery_rejects_a_valid_commit_beyond_the_next_sequence() {
        let root = test_root();
        let ledger = Ledger::open(&root, &mut OsSecureRandom).unwrap();
        let future_head = LedgerHead {
            sequence: 1,
            root: LedgerHead::genesis().root(),
        };
        let (record, _) =
            build_pending_record(ledger.identity(), future_head, SessionId::new([0xA7; 32]));
        write_private_record(
            &root
                .join(identity::RECORDS_DIR_NAME)
                .join("future.controller.yonaudit"),
            &record,
        );
        assert!(matches!(
            Ledger::open(&root, &mut OsSecureRandom),
    ...
    #[test]
    fn recovery_without_a_records_directory_preserves_the_head() {
        let root = test_root();
        let ledger = Ledger::open(&root, &mut OsSecureRandom).unwrap();
        fs::remove_dir(root.join(identity::RECORDS_DIR_NAME)).unwrap();
        assert_eq!(
            recover_pending_records(&root, ledger.identity(), ledger.head()).unwrap(),
            ledger.head()
        );

        fs::write(root.join(identity::RECORDS_DIR_NAME), b"not a directory").unwrap();
        assert!(matches!(
            recover_pending_records(&root, ledger.identity(), ledger.head()),
            Err(AuditLedgerError::RecordReadFailed(_))
        ));

        assert!(matches!(
            open_lock_file(Path::new("invalid\0ledger-root")),
            Err(AuditLedgerError::LockFailed(_))
        ));
        assert!(matches!(
            load_or_create_state(Path::new("invalid\0ledger-root")),
            Err(AuditLedgerError::StateReadFailed(_))
        ));
    }
