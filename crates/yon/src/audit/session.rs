...
//! (`/yonder/audit/3.0.0`, design sections 13.1 and 28) and persists the
...
//!
//! ```text
//! SHA-256("yonder-audit-chain-local-v3" || previous_local_hash
...
//!   local commit ambiguity: event_kind || transfer_id(u64)
//!   || final_size(u64) || digest[32] || local_path_len(u16 = 0)
...
pub const INPUT_COMMITMENT_LABEL: &[u8] = b"yonder-audit-input-commitment-v3";
/// The session ID binding label (design section 13.4).
pub const SESSION_ID_LABEL: &[u8] = b"yonder-audit-session-v3";
/// Shared input chain domain label (design sections 16.2 and 17.3).
pub const CHAIN_INPUT_DOMAIN: &[u8] = b"yonder-audit-chain-input-v3";
/// Shared output chain domain label (design sections 16.3 and 17.3).
pub const CHAIN_OUTPUT_DOMAIN: &[u8] = b"yonder-audit-chain-output-v3";
/// Shared terminal control chain domain label (design sections 15.2 and 17.3).
pub const CHAIN_CONTROL_DOMAIN: &[u8] = b"yonder-audit-chain-control-v3";
/// Shared file transfer chain domain label (design sections 18.6 and 17.3).
pub const CHAIN_FILE_DOMAIN: &[u8] = b"yonder-audit-chain-file-v3";
/// Local observation chain domain label (design sections 15.3 and 17.3).
pub const CHAIN_LOCAL_DOMAIN: &[u8] = b"yonder-audit-chain-local-v3";
...
/// Local-only outcome: the receiver committed the file, but could not
/// deliver the `Committed` acknowledgement. This value must never enter the
/// shared file-transfer chain because the sender cannot verify it.
pub const FILE_LOCAL_KIND_COMMITTED_UNCONFIRMED: u8 = 0x05;
/// Local-only outcome: the sender delivered `Finish`, then lost the exchange
/// before learning whether the receiver committed the file. This value must
/// never enter the shared file-transfer chain.
pub const FILE_LOCAL_KIND_COMMIT_STATUS_UNKNOWN: u8 = 0x06;
/// Exact kind-specific payload length of a local-only commit ambiguity
/// record. Such a record deliberately carries a zero-length path.
pub(crate) const LOCAL_FILE_AMBIGUITY_PAYLOAD_LEN: usize = 1 + 8 + 8 + DIGEST_LEN + 2;
...
/// Local effect confirmation values (design section 18).
///
/// `UNCONFIRMED` deliberately does not assert that no bytes reached the
/// destination. A cancelled or timed-out operating-system write can have
/// completed partially, or can complete after its future is dropped.
pub const OUTCOME_CONFIRMED: u8 = 0x01;
pub const OUTCOME_UNCONFIRMED: u8 = 0x02;
...
impl<'a> RecordBatch<'a> {
    ...
    /// Consumes the batch and yields its records in append order. Heap
    /// payloads retain their allocation so the bounded writer queue can take
    /// ownership without cloning them.
    pub fn into_records(self) -> impl Iterator<Item = EncodedRecord<'a>> {
        self.records
            .into_iter()
            .take(self.len)
            .map(|record| record.expect("every slot below the batch length is filled"))
    }
...
/// The latest shared file event to which a local path or post-commit
/// observation may be related. Keeping the wire facts beside the head makes
/// invalid role/direction and transfer-ID combinations unrepresentable in a
/// successfully recorded local ambiguity result.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
/// The only valid local ambiguity kind for one endpoint role and transfer
/// direction. Upload senders and download senders cannot know the receiver's
/// commit status; the corresponding receivers can know a local commit whose
/// acknowledgement was not delivered.
pub(crate) const fn expected_local_file_ambiguity_kind(
    role: AuditRole,
    direction: u8,
) -> Option<u8> {
    match (role, direction) {
        (AuditRole::Controller, FILE_DIRECTION_UPLOAD)
        | (AuditRole::Host, FILE_DIRECTION_DOWNLOAD) => Some(FILE_LOCAL_KIND_COMMIT_STATUS_UNKNOWN),
        (AuditRole::Host, FILE_DIRECTION_UPLOAD)
        | (AuditRole::Controller, FILE_DIRECTION_DOWNLOAD) => {
            Some(FILE_LOCAL_KIND_COMMITTED_UNCONFIRMED)
        }
        _ => None,
    }
}
...
impl AuditSession {
    ...
    pub fn record_send_outcome<'a>(
        ...
        append(&mut kind_payload, &mut cursor, &[outcome_code(confirmed)]);
    ...
    pub fn record_shared_close_reason<'a>(
        ...
        append(&mut kind_payload, &mut cursor, &[OUTCOME_CONFIRMED]);
        self.push_local(
            &mut batch,
    ...
    pub fn record_file_transfer<'a>(
        ...
        if !matches!(
            facts.direction,
            FILE_DIRECTION_UPLOAD | FILE_DIRECTION_DOWNLOAD
        ) || !matches!(
            facts.kind,
        ...
        if local_path.is_some_and(|path| path.len() > MAX_LOCAL_PATH_LEN) {
            return Err(AuditError::SegmentTooLarge);
        }
        let mut batch = RecordBatch::new();
        ...
        // All fallible validation and batch construction has completed. The
        // shared state and its local association now advance atomically from
        ...
        self.latest_file_event = Some(LatestFileEvent {
            ...
            kind: facts.kind,
            local_path_recorded: local_path.is_some(),
            ambiguity_recorded: false,
        });
        self.checkpoint_shared_bytes += payload_len as u64;
        // A file START and every shared terminal result are operator-visible
        // boundaries and therefore require an immediate bilateral checkpoint.
        self.checkpoint_due_flag = true;
        Ok(batch)
    }

    /// Records a local-only post-`Finish` result that cannot truthfully enter
    /// the shared chain. It is related to the latest shared file-transfer
    /// block (normally the matching start event) and deliberately carries no
    /// path: the transfer ID identifies the protocol operation while keeping
    /// this API usable on platforms whose local path is not UTF-8.
    pub fn record_local_file_transfer_result<'a>(
        &mut self,
        kind: u8,
        transfer_id: u64,
        final_size: u64,
        ...
        if !matches!(
            kind,
            FILE_LOCAL_KIND_COMMITTED_UNCONFIRMED | FILE_LOCAL_KIND_COMMIT_STATUS_UNKNOWN
        ) {
            return Err(AuditError::InvalidState(
                "the local file transfer result kind is invalid",
            ));
        }
        if local_path.is_some_and(|path| path.len() > MAX_LOCAL_PATH_LEN) {
            return Err(AuditError::SegmentTooLarge);
        }
        let latest = self.latest_file_event.ok_or(AuditError::InvalidState(
            "a local file result requires a shared transfer start",
        ))?;
        if latest.kind != FILE_KIND_START
            || latest.transfer_id != transfer_id
            || latest.ambiguity_recorded
            || expected_local_file_ambiguity_kind(self.role, latest.direction) != Some(kind)
        {
            return Err(AuditError::InvalidState(
                "the local file transfer result does not match the active transfer",
            ));
        }
        let mut payload = [0_u8; LOCAL_FILE_AMBIGUITY_PAYLOAD_LEN];
        payload[0] = kind;
        payload[1..9].copy_from_slice(&transfer_id.to_be_bytes());
        payload[9..17].copy_from_slice(&final_size.to_be_bytes());
        payload[17..49].copy_from_slice(digest.as_bytes());
        // payload[49..51] is the mandatory zero-length local path.
        let mut batch = RecordBatch::new();
        let record_path = local_path.is_some() && !latest.local_path_recorded;
        if let Some(local_path) = local_path.filter(|_| record_path) {
            let mut path_payload = vec![0_u8; 1 + 8 + 2 + local_path.len()];
            let mut cursor = 0;
            append(&mut path_payload, &mut cursor, &[FILE_KIND_START]);
            write_u64(&mut path_payload, &mut cursor, transfer_id);
            write_u16(&mut path_payload, &mut cursor, local_path.len() as u16);
            append(&mut path_payload, &mut cursor, local_path.as_bytes());
            self.push_local(
                &mut batch,
                RecordType::LocalFileTransferEvent,
                &path_payload,
        ...
        latest.local_path_recorded |= record_path;
        latest.ambiguity_recorded = true;
        self.checkpoint_due_flag = true;
        Ok(batch)
    }
    ...
    pub fn fail_closed_records<'a>(
        ...
        let mut cursor = 0;
        append(&mut kind_payload, &mut cursor, &[reason.code()]);
        append(&mut kind_payload, &mut cursor, &[OUTCOME_UNCONFIRMED]);
        self.push_local(
            &mut batch,
            RecordType::LocalCloseEvent,
            &kind_payload,
    ...
    /// Encodes one local record, pushes it into the batch and then advances
    /// the local chain. The record payload is `time || related || payload`;
    /// no chain state changes if encoding or batch insertion fails.
    fn push_local(
        &mut self,
        ...
        let payload_len = SPLIT_PREFIX_LEN + kind_payload.len();
        if payload_len <= MAX_INLINE_PAYLOAD_LEN {
            let mut payload = [0_u8; MAX_INLINE_PAYLOAD_LEN];
            let mut cursor = 0;
            append(
                &mut payload,
                &mut cursor,
            ...
            append(&mut payload, &mut cursor, kind_payload);
            batch.push_boxed(record_type, payload.into_boxed_slice())?;
        }
        self.commit_local(record_type, kind_payload, &related, now_ns);
        Ok(())
    }
...
fn outcome_code(confirmed: bool) -> u8 {
    if confirmed {
        OUTCOME_CONFIRMED
    } else {
        OUTCOME_UNCONFIRMED
...
mod tests {
    ...
    fn receive_peer_hello_rejects_invalid_peers() {
        ...
        // Unsupported format (version two is structurally decodable but
        // not offered by this endpoint).
        let mut bytes = hello_h.encode_payload().as_slice().to_vec();
        bytes[169..171].copy_from_slice(&2_u16.to_be_bytes());
    ...
    fn receive_peer_ready_rejects_invalid_confirmations() {
        ...
        // Wrong format (version two is structurally decodable but not
        // offered by this endpoint).
        let mut bytes = ready_h.encode_payload().as_slice().to_vec();
        bytes[64..66].copy_from_slice(&2_u16.to_be_bytes());
    ...
    #[test]
    fn closing_partial_blocks_can_cross_the_checkpoint_size_boundary() {
        let binding = test_binding(0xA1);
        let mut input = test_session(AuditRole::Controller, 29, binding);
        let mut input_peer = test_session(AuditRole::Host, 129, binding);
        handshake(&mut input, &mut input_peer);
        input.checkpoint_shared_bytes = CHECKPOINT_SIZE_TRIGGER - 1;
        input.record_input(b"x", 1).unwrap();
        assert!(!input.checkpoint_due(1));
        input.close_directions().unwrap();
        assert!(input.checkpoint_due(1));

        let mut output = test_session(AuditRole::Controller, 30, binding);
        let mut output_peer = test_session(AuditRole::Host, 130, binding);
        handshake(&mut output, &mut output_peer);
        output.checkpoint_shared_bytes = CHECKPOINT_SIZE_TRIGGER - 1;
        output.record_output(b"x", 1).unwrap();
        assert!(!output.checkpoint_due(1));
        output.close_directions().unwrap();
        assert!(output.checkpoint_due(1));
    }
    ...
    #[test]
    fn terminal_exit_audit_preserves_the_full_cross_platform_code() {
        let binding = test_binding(0x63);
        let mut controller = test_session(AuditRole::Controller, 22, binding);
        let mut host = test_session(AuditRole::Host, 122, binding);
        handshake(&mut controller, &mut host);

        for code in [255_u32, 256, 0xC000_013A, u32::MAX] {
            let batch = controller
                .record_terminal_exit(code, u64::from(code))
                .unwrap();
            let shared = shared_payloads(&batch, RecordType::SharedControlEvent);
            let control = decode_shared_control(&shared[0]).unwrap();
            assert_eq!(control.kind, CONTROL_KIND_TERMINAL_EXIT);
            assert_eq!(control.control_payload, code.to_be_bytes());
        }
    }

    #[test]
    fn uncertain_file_commit_results_are_local_and_related_to_the_shared_start() {
        fn start_for(role: AuditRole, direction: u8, transfer_id: u64) -> AuditSession {
            let binding = test_binding(0x66);
            let mut controller = test_session(AuditRole::Controller, 26, binding);
            let mut host = test_session(AuditRole::Host, 126, binding);
            handshake(&mut controller, &mut host);
        ...
        let mut host = test_session(AuditRole::Host, 126, binding);
        handshake(&mut controller, &mut host);
        let digest = Digest32::new([9; DIGEST_LEN]);

        assert_eq!(
            expected_local_file_ambiguity_kind(AuditRole::Controller, 0xFF),
            None
        );

        assert!(matches!(
            controller.record_local_file_transfer_result(
        ...
        assert!(matches!(
            controller.record_local_file_transfer_result(0xFF, 7, 1000, digest, None, 1),
            Err(AuditError::InvalidState(
                "the local file transfer result kind is invalid"
            ))
        ));

        let mut controller = start_for(AuditRole::Controller, FILE_DIRECTION_UPLOAD, 7);
        let oversized_local_path = "l".repeat(MAX_LOCAL_PATH_LEN + 1);
        assert!(matches!(
            controller.record_local_file_transfer_result(
                FILE_LOCAL_KIND_COMMIT_STATUS_UNKNOWN,
                7,
                1000,
                digest,
                Some(&oversized_local_path),
                2,
            ),
            Err(AuditError::SegmentTooLarge)
        ));
        assert!(controller.checkpoint_due(2));
        controller.build_checkpoint(2).unwrap();
        assert!(!controller.checkpoint_due(2));
        let start_head = controller.last_block[SharedStream::FileTransfer.index()].unwrap();
        let before = (
            controller.shared_snapshot(),
            controller.local_chain_head(),
            controller.local_event_count(),
        );
        assert!(
            controller
        ...
        let batch = controller
            ...
            .unwrap();
        assert!(controller.checkpoint_due(4));
        let records = batch_payload(&batch);
        let payload = &records[0].1;
        assert_eq!(records[0].0, RecordType::LocalFileTransferEvent);
        assert_eq!(&payload[8..SPLIT_PREFIX_LEN], start_head.as_bytes());
        assert_eq!(
            payload[SPLIT_PREFIX_LEN],
            FILE_LOCAL_KIND_COMMIT_STATUS_UNKNOWN
        );
        assert_eq!(
            &payload[SPLIT_PREFIX_LEN + 1..SPLIT_PREFIX_LEN + 9],
            &7_u64.to_be_bytes()
        );
        assert_eq!(
            &payload[SPLIT_PREFIX_LEN + 9..SPLIT_PREFIX_LEN + 17],
            &1000_u64.to_be_bytes()
        );
        assert_eq!(
            &payload[SPLIT_PREFIX_LEN + 17..SPLIT_PREFIX_LEN + 49],
            digest.as_bytes()
        );
        assert_eq!(&payload[SPLIT_PREFIX_LEN + 49..], &[0, 0]);
        assert_eq!(
            payload.len(),
            SPLIT_PREFIX_LEN + LOCAL_FILE_AMBIGUITY_PAYLOAD_LEN
        );
        assert!(
            controller
                .record_local_file_transfer_result(
        ...
        for (role, direction, expected, rejected) in [
            (
                AuditRole::Controller,
            ...
            let mut session = start_for(role, direction, 11);
            assert!(
                session
                    .record_local_file_transfer_result(rejected, 11, 1000, digest, None, 3)
                    .is_err()
            );
            session
                .record_local_file_transfer_result(
                    ...
                    1000,
                    digest,
                    Some("/local/resolved.bin"),
                    4,
                )
                .unwrap();
        }

        let mut terminal = start_for(AuditRole::Controller, FILE_DIRECTION_UPLOAD, 9);
        terminal.build_checkpoint(2).unwrap();
        assert!(!terminal.checkpoint_due(2));
        terminal
            .record_file_transfer(
                &FileTransferFacts {
                    transfer_id: 9,
            ...
            .unwrap();
        assert!(terminal.checkpoint_due(3));
        assert!(
            terminal
                .record_local_file_transfer_result(
                    FILE_LOCAL_KIND_COMMIT_STATUS_UNKNOWN,
    ...
    fn file_transfer_audit_rejects_every_unbounded_path_field() {
        ...
        let mut invalid = facts("/ok", "ok");
        invalid.direction = 0xFF;
        assert!(controller.record_file_transfer(&invalid, None, 4).is_err());
        invalid.direction = FILE_DIRECTION_DOWNLOAD;
        invalid.kind = FILE_LOCAL_KIND_COMMITTED_UNCONFIRMED;
        assert!(controller.record_file_transfer(&invalid, None, 5).is_err());
        assert_eq!(
            before,
            (
                controller.shared_snapshot(),
        ...
        let longest_valid_local_path = "l".repeat(MAX_LOCAL_PATH_LEN);
        let batch = controller
            .record_file_transfer(&facts("/ok", "ok"), Some(&longest_valid_local_path), 6)
            .unwrap();
        assert!(matches!(
            batch.iter().nth(1).map(|record| &record.payload),
            Some(Payload::Boxed(_))
        ));
    }
