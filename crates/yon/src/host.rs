...
mod tests {
    ...
    use crate::audit::session::{
        DIRECTION_CTRL_TO_HOST, FILE_DIRECTION_DOWNLOAD, FILE_DIRECTION_UPLOAD, FILE_KIND_START,
        FILE_KIND_SUCCESS, FileTransferFacts,
    };
    use crate::file_semantics::BaseDirectory;
                ...
                runtime.block_on(async {
                    ...
                    tokio::time::timeout(
                        ...
                        local.run_until(async {
                            ...
                            let controller_audit_root = tempdir().unwrap().path().join("audit");
                            let host_audit_root = tempdir().unwrap().path().join("audit");
                            ...
                            let controller = tokio::task::spawn_local(async move {
                                ...
                                let audit = AuditObserver::establish(
                                    audit_stream.into_tokio(),
                                    AuditRole::Controller,
                                    controller.node.peer_id(),
                                    host_peer,
                                ...
                                // The controller input records (design section 18.1) so
                                // the shared chains match the host's receive records.
                            ...
                            let binding = wait_for_controller_quiescence(
                                &mut driver,
                                &mut incoming,
                                controller_peer,
                            )
                            ...
                            let (mut pty, base, data, control, audit) = drive_session_inputs(
                                &mut driver,
                                binding,
                                &mut incoming,
                                start_terminal(
                            ...
                            // The host-side audit finalization (design sections 21 and
                            // 22.1). The driver is polled throughout so the peer's
                            // finalization frames arrive.
                            let finalize = audit
                                .as_ref()
                                .expect("enterprise mode must establish audit")
                                .close_and_finalize(
                                    ManifestEnding::ShellExit(EXIT_CODE as u8),
                                    true,
                            ...
                            loop {
                                tokio::select! {
                                    result = &mut finalize => {
                                        result.expect("the host audit finalization must complete");
                                        break;
                                    }
                                    _ = driver.next() => {}
                                }
    ...
    async fn run(
        ...
        loop {
            ...
            let audit_root = match relay_access {
                ...
                RelayAccessMode::Enterprise => Some(crate::audit::observer::platform_audit_root()?),
            };
            ...
            event = pty.next() => {
                ...
                match event.kind() {
                    PtyEventKind::Output => {
                        ...
                        if let Err(error) = data_write.write_all(output.as_slice()).await {
                            if let Some(audit) = audit {
                                let _ =
