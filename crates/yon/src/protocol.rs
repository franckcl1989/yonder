...
/// Resolves the locator through the exact access policy configured by the
/// endpoint. Enterprise endpoints never probe or fall back to the standard
/// resolve protocol; a relay that omits the enterprise protocol fails closed.
pub async fn resolve_peer_for_mode(
    driver: &mut EndpointDriver,
    streams: &mut Libp2pApplicationStreams,
    relay: &RelayConnection,
    locator: Locator,
    ...
    match expected {
        RelayAccessMode::Standard => resolve_peer(driver, streams, relay, locator, deadline)
            .await
            .map(|peer| ResolvedTarget::new(peer, RelayAccessMode::Standard)),
        RelayAccessMode::Enterprise => {
            enterprise_resolve_peer(driver, streams, relay, locator, deadline, ui)
                .await
                .map(|peer| ResolvedTarget::new(peer, RelayAccessMode::Enterprise))
        }
    }
}
...
/// `UnsupportedProtocol`; callers configured for enterprise access must
/// surface that failure and never retry the legacy resolve protocol.
///
...
mod tests {
    use super::{
        ...
        read_enterprise_response, reclaim_response, release_response, resolve_response,
        retry_delay,
    };
    use std::io;
    use std::pin::Pin;
    ...
    impl FailingUi {
        const fn new(failure: UiFailure) -> Self {
            Self {
                failure,
                offered: false,
                opened: false,
    ...
    impl EnterpriseResolveUi for FailingUi {
        fn select_provider(
            &mut self,
        ...
        > {
            Box::pin(async move {
                self.offered = true;
                match self.failure {
                    UiFailure::Selection => Err(RelayProtocolError::EnterpriseRejected),
                    UiFailure::Authorization => Ok(EnterpriseProvider::WeCom),
                }
            })
        }
    ...
    async fn relay_enterprise_until_ui_failure(
        mut stream: impl AsyncRead + AsyncWrite + Unpin,
        failure: UiFailure,
    ) {
        let mut start = [0_u8; 4];
        stream.read_exact(&mut start).await.unwrap();
        assert!(EnterpriseStart::decode(&start).is_ok());
        stream
            .write_all(
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn enterprise_exchange_fails_closed_when_the_ui_rejects_a_step() {
        for failure in [UiFailure::Selection, UiFailure::Authorization] {
            let (mut client, server) = tokio::io::duplex(1024);
            let relay_side = relay_enterprise_until_ui_failure(server, failure);
            let mut ui = FailingUi::new(failure);
            let attempt = async {
                enterprise_exchange_io(
                    &mut client,
                    Locator::new(7).unwrap(),
            ...
            assert!(ui.offered);
            assert_eq!(ui.opened, failure == UiFailure::Authorization);
        }
    }

    #[tokio::test(flavor = "current_thread")]
    async fn enterprise_response_reader_rejects_every_truncated_shape_and_unknown_tag() {
        for response in [
            &[][..],
            &[0x10][..],
            &[0x11, 0, 0, 0][..],
            ...
            server.shutdown().await.unwrap();
            assert!(matches!(
                read_enterprise_response(&mut client, Duration::from_secs(1)).await,
                Err(RelayProtocolError::Io(error))
        ...
        assert!(matches!(
            read_enterprise_response(&mut client, Duration::from_secs(1)).await,
            Err(RelayProtocolError::Protocol(_))
        ));
    }
