use crate::audit::PendingAuditStep;
...
use crate::file_semantics::{BaseDirectory, FileTransferBackend, TokioFileTransferBackend};
...
use crate::progress::{HEARTBEAT_INTERVAL, NoopProgress, OperationProgress, wait_with_progress};
use crate::protocol::{
    EnterpriseResolveUi, RelayProtocolError, ResolveDeadline, ResolvedTarget, resolve_peer_for_mode,
};
...
const LOCAL_OUTPUT_TIMEOUT: Duration = Duration::from_secs(2);
...
trait TerminalFrontend {
    ...
    type Output: tokio::io::AsyncWrite + Send + Unpin;
    type RawModeGuard;
...
impl RemoteTerminalOutput {
    ...
    fn prepare_into(&mut self, bytes: &[u8], display: &mut Vec<u8>) {
        display.clear();
        match self.mode {
            RemoteTerminalOutputMode::Bytes => display.extend_from_slice(bytes),
            RemoteTerminalOutputMode::WindowsConsoleUtf8 => {
                self.prepare_windows_console_utf8(bytes, display);
            }
        }
    }

    fn prepare_windows_console_utf8(&mut self, mut bytes: &[u8], display: &mut Vec<u8>) {
    ...
    fn prepare(&mut self, bytes: &[u8]) -> Vec<u8> {
        let mut display = Vec::with_capacity(bytes.len() * UTF8_REPLACEMENT.len());
        self.prepare_into(bytes, &mut display);
        display
    }
    ...
    /// Takes the display tail for an incomplete Windows-console UTF-8
    /// sequence. This is deliberately pure: the caller must append the
    /// display audit record before producing the external write effect.
...
const TERMINAL_STEP_AUDIT: u8 = 0;
const TERMINAL_STEP_IO: u8 = 1;

/// Cancellation classification for an in-flight terminal side effect.
/// Audit phases must retain their sole owner through completion; only the
/// external I/O phase may be cut off by the shared Closing deadline.
struct TerminalStepPhase(AtomicU8);

impl TerminalStepPhase {
    const fn audit() -> Self {
        Self(AtomicU8::new(TERMINAL_STEP_AUDIT))
    }

    fn enter_audit(&self) {
        self.0.store(TERMINAL_STEP_AUDIT, Ordering::Release);
    }

    fn enter_io(&self) {
        self.0.store(TERMINAL_STEP_IO, Ordering::Release);
    }

    fn is_io(&self) -> bool {
        self.0.load(Ordering::Acquire) == TERMINAL_STEP_IO
    }
}
...
pub struct EnterpriseControllerUi<I: AsyncRead + Unpin, O: AsyncWrite + Unpin> {
    ...
    progress: Option<mpsc::Sender<EnterpriseUiProgress>>,
}

enum EnterpriseUiProgress {
    Pause(oneshot::Sender<()>),
    WaitingForAuthorization(oneshot::Sender<()>),
}
...
impl EnterpriseControllerUi<tokio::io::BufReader<tokio::io::Stdin>, tokio::io::Stdout> {
    ...
    fn with_progress(progress: mpsc::Sender<EnterpriseUiProgress>) -> Self {
        Self {
            input: tokio::io::BufReader::new(tokio::io::stdin()),
            output: tokio::io::stdout(),
            opener: Box::new(platform_open),
...
async fn update_enterprise_progress(
    progress: Option<mpsc::Sender<EnterpriseUiProgress>>,
    command: impl FnOnce(oneshot::Sender<()>) -> EnterpriseUiProgress,
    ...
    progress
        ...
        .await
        .map_err(|_| RelayProtocolError::EnterpriseRejected)?;
    acknowledged
        .await
        .map_err(|_| RelayProtocolError::EnterpriseRejected)
}
    ...
    for EnterpriseControllerUi<I, O>
{
    ...
    > {
        ...
        Box::pin(async move {
            update_enterprise_progress(progress, EnterpriseUiProgress::Pause).await?;
            output
                .write_all(b"Select enterprise authentication provider:\n1) WeCom\n2) Feishu\n> ")
                .await
            ...
            loop {
                ...
                input
                    ...
                    .map_err(RelayProtocolError::Io)?;
                match line.trim() {
                    "1" => return Ok(EnterpriseProvider::WeCom),
                    "2" => return Ok(EnterpriseProvider::Feishu),
                    _ => {
                        output
                            .write_all(b"Invalid selection. Enter 1 or 2.\n> ")
                            .await
                            .map_err(RelayProtocolError::Io)?;
                        output.flush().await.map_err(RelayProtocolError::Io)?;
                    }
    ...
    fn open_authorization(
        ...
        let progress = self.progress.clone();
        Box::pin(async move {
            update_enterprise_progress(progress.clone(), EnterpriseUiProgress::Pause).await?;
            output
                .write_all(
                    format!("\nComplete enterprise authorization in your browser:\n{url}\n")
                        .as_bytes(),
                )
                .await
                .map_err(RelayProtocolError::Io)?;
            if !opener(&url) {
                output
                    .write_all(b"The browser could not be opened; use the URL shown above.\n")
                    .await
                    .map_err(RelayProtocolError::Io)?;
            }
            output.flush().await.map_err(RelayProtocolError::Io)?;
            update_enterprise_progress(progress, EnterpriseUiProgress::WaitingForAuthorization)
                .await?;
            Ok(())
        })
    }
...
pub struct ControllerConfig {
    ...
    expected_access: RelayAccessMode,
}
...
pub enum ControllerStage {
    ConnectingRelay,
    ResolvingHost,
    WaitingForEnterpriseAuthorization,
    EstablishingPath,
    RelayFallback,
    Authenticating,
    StartingTerminal,
...
async fn resolve_configured_target(
    ...
    let mut ui = EnterpriseControllerUi::with_progress(progress_tx);
    let resolve = resolve_peer_for_mode(
        driver,
        streams,
        relay,
        locator,
    ...
    drive_enterprise_resolve_progress(progress, progress_rx, resolve).await
}

async fn drive_enterprise_resolve_progress<T>(
    progress: &mut impl OperationProgress<ControllerStage>,
    mut progress_rx: mpsc::Receiver<EnterpriseUiProgress>,
    resolve: impl Future<Output = Result<T, RelayProtocolError>>,
) -> Result<T, RelayProtocolError> {
    tokio::pin!(resolve);
    let mut visible_stage = Some(ControllerStage::ResolvingHost);
    ...
    loop {
        tokio::select! {
            ...
            command = progress_rx.recv(), if commands_open => match command {
                Some(EnterpriseUiProgress::Pause(acknowledge)) => {
                    progress.clear();
                    visible_stage = None;
                    let _ = acknowledge.send(());
                }
                Some(EnterpriseUiProgress::WaitingForAuthorization(acknowledge)) => {
                    let stage = ControllerStage::WaitingForEnterpriseAuthorization;
                    progress.update(stage);
                    visible_stage = Some(stage);
                    let _ = acknowledge.send(());
                }
                None => commands_open = false,
            },
            _ = heartbeat.tick(), if visible_stage.is_some() => {
                progress.update(visible_stage.expect("heartbeat branch requires a visible stage"));
            }
        }
    }
}
...
async fn prepare_controller_session(
    ...
    let resolved = resolve_configured_target(
        &mut driver,
        &mut streams,
        &relay,
        code.locator(),
...
async fn run_terminal<F: TerminalFrontend>(
    ...
    let (mut data_read, data_write) = tokio::io::split(data.into_tokio());
    let mut data_write = Some(data_write);
    ...
    let mut output = Some(frontend.output());
    let mut control_write = Some(control_write);
    ...
    let mut deferred_terminal_close = false;
    let session = {
        ...
        let mut resize_poll = tokio::time::interval(SIZE_POLL_INTERVAL);
        resize_poll.set_missed_tick_behavior(tokio::time::MissedTickBehavior::Skip);
        let mut last_terminal_size = hello.size();
        let mut audit_frames = Box::pin(wait_for_audit_frame(audit.as_ref()));
        let mut audit_poll = tokio::time::interval(AUDIT_CHECKPOINT_POLL);
        audit_poll.set_missed_tick_behavior(tokio::time::MissedTickBehavior::Skip);
        ...
        let mut audit_send: Option<PendingAuditStep<'_, ()>> = None;
        let mut audit_process: Option<PendingAuditStep<'_, FrameEvent>> = None;
        let local_input_phase = TerminalStepPhase::audit();
        let remote_write_phase = TerminalStepPhase::audit();
        let resize_phase = TerminalStepPhase::audit();
        let terminal_closing = AtomicBool::new(false);
        let mut display_buffer = Vec::with_capacity(
            TerminalChunk::capacity()
                .checked_mul(UTF8_REPLACEMENT.len())
                .expect("the terminal display capacity is bounded"),
        );
        let mut local_input_step = None;
        let mut local_input_active = false;
        let mut local_input_length = 0_u64;
        let mut remote_output_step = None;
        let mut remote_output_active = false;
        let mut remote_write_step = None;
        let mut remote_write_active = false;
        let mut remote_write_length = 0_u64;
        let mut resize_step = None;
        let mut resize_active = false;
        let mut terminal_exit_audit: Option<PendingAuditStep<'_, ()>> = None;
        let mut terminal_exit_code = None;
        let mut cancel_action_audit: Option<PendingAuditStep<'_, ()>> = None;
        let mut audit_failure: Option<Pin<Box<dyn Future<Output = ()> + '_>>> = None;
        let mut pending_local_input = None;
        let mut pending_local_eof = false;
        let mut pending_transfer = None;
        let mut pending_completion = None;
        let mut completed_code = None;
        tokio::pin!(remote_exit);
        let mut outcome = loop {
            let owners_idle = audit_send.is_none()
                && audit_process.is_none()
                && terminal_exit_audit.is_none()
                && cancel_action_audit.is_none()
                && audit_failure.is_none()
                && !local_input_active
                && !remote_output_active
                && !remote_write_active
                && !resize_active
                && transfer.is_none();

            if owners_idle {
                if let Some(transfer_outcome) = pending_transfer.take() {
                    match handle_transfer_event(
                        transfer_outcome,
                        output.as_mut().expect("remote display writer is idle"),
                        &mut session_ui,
                        &transfer_cancel,
                        &mut transfer,
                        audit.as_ref(),
                    ...
                    .await
                    {
                        Ok(completion) => pending_completion = completion,
                        Err(error) => break Err(error),
                    }
                    continue;
                }
                if pending_local_eof {
                    pending_local_eof = false;
                    match finish_local_input_eof(
                        data_write.as_mut().expect("the local-input writer is idle"),
                        output.as_mut().expect("remote display writer is idle"),
                        &mut terminal_output,
                        &mut session_ui,
                        audit.as_ref(),
                ...
                if let Some(processed) = pending_local_input.take() {
                ...
                if let Some(code) = pending_completion.take() {
                    ...
                    .await
                    {
                        Ok(()) => completed_code = Some(code),
                        Err(error) => break Err(error),
                    }
                    continue;
                }
            }
            // Terminal completion starts Closing, but an audit step that has
            // already advanced durable state remains the sole owner of that
            // exchange until it settles. Only then may finalization take over
            // the ordered audit substream.
            if let Some(code) = completed_code
                && audit_send.is_none()
                && audit_process.is_none()
                && !local_input_active
                && !remote_output_active
            ...
            tokio::select! {
                ...
                event = async {
                    tokio::select! {
                        ...
                        result = &mut audit_frames,
                            if audit.is_some() && audit_send.is_none() && audit_process.is_none() => {
                            TerminalPumpEvent::AuditFrame(result)
                        }
                        _ = audit_poll.tick(),
                            if audit.is_some() && audit_send.is_none() && audit_process.is_none() => {
                            TerminalPumpEvent::AuditPoll
                        }
                        result = async {
                        ...
                        () = async {
                            audit_failure
                                .as_mut()
                                .expect("the audit-failure branch is gated on the slot")
                                .await;
                            audit_failure = None;
                        }, if audit_failure.is_some() => TerminalPumpEvent::AuditFailureHandled,
                        result = async {
                            let outcome = terminal_exit_audit
                                .as_mut()
                                .expect("the terminal-exit audit branch is gated on the slot")
                                .await;
                            terminal_exit_audit = None;
                            outcome
                        }, if terminal_exit_audit.is_some() => {
                            TerminalPumpEvent::RemoteExitAudited(result)
                        }
                        result = async {
                            let outcome = cancel_action_audit
                                .as_mut()
                                .expect("the transfer-cancel audit branch is gated on the slot")
                                .await;
                            cancel_action_audit = None;
                            outcome
                        }, if cancel_action_audit.is_some() => {
                            TerminalPumpEvent::TransferCancelAudited(result)
                        }
                        result = async {
                            let (writer, outcome) = local_input_step
                                .as_mut()
                                .expect("the local-input branch is gated on the slot")
                                .await;
                            local_input_active = false;
                            data_write = Some(writer);
                            outcome
                        }, if local_input_active && output.is_some() => {
                            TerminalPumpEvent::LocalInputProcessed(result)
                        }
                        result = read_local_input(
                            &mut input,
                            local_input_reserve(&session_ui.control),
                        ), if !session_ui.local_ended
                            && completed_code.is_none()
                            && !local_input_active
                            && output.is_some() => {
                            TerminalPumpEvent::LocalInputRead(result)
                        }
                        result = async {
                            let outcome = remote_output_step
                                .as_mut()
                                .expect("the remote-output branch is gated on the slot")
                                .await;
                            remote_output_active = false;
                            outcome
                        }, if remote_output_active => {
                            TerminalPumpEvent::RemoteOutputProcessed(result)
                        }
                        result = async {
                            let outcome = remote_write_step
                                .as_mut()
                                .expect("the remote-write branch is gated on the slot")
                                .await;
                            remote_write_active = false;
                            outcome
                        }, if remote_write_active => {
                            TerminalPumpEvent::RemoteOutputWritten(result)
                        }
                        result = async {
                            let (writer, outcome) = resize_step
                                .as_mut()
                                .expect("the terminal-resize branch is gated on the slot")
                                .await;
                            resize_active = false;
                            control_write = Some(writer);
                            outcome
                        }, if resize_active => TerminalPumpEvent::ResizeProcessed(result),
                        result = read_remote_output_chunk(&mut data_read), if remote.output_open()
                            && !remote_output_active
                            && !remote_write_active => {
                            TerminalPumpEvent::RemoteOutputRead(result)
                        }
                        ...
                        resize = poll_terminal_resize(
                            &frontend,
                            &mut resize_poll,
                            last_terminal_size,
                            interactive,
                        ), if !resize_active && completed_code.is_none() => {
                            TerminalPumpEvent::ResizeDetected(resize)
                        }
                        result = async {
                            let outcome = poll_inflight_transfer(&mut transfer).await;
                            transfer = None;
                            outcome
                        }, if transfer.is_some() && pending_transfer.is_none() => {
                    ...
                    let completion = match event {
                        ...
                        TerminalPumpEvent::LocalInputRead(result) => match result {
                            Ok(Some(chunk)) => {
                                tracing::debug!(
                                    length = chunk.as_slice().len(),
                                    "local terminal input read completed"
                                );
                                let processed = session_ui.control.process(chunk);
                                local_input_length = processed.remote_bytes.as_slice().len() as u64;
                                let writer = data_write
                                    .take()
                                    .expect("the local-input writer is idle");
                                let step = forward_processed_input_owned(
                                    writer,
                                    processed,
                                    audit.as_ref(),
                                    &local_input_phase,
                                ...
                                arm_reusable_step(&mut local_input_step, step);
                                local_input_active = true;
                                None
                            }
                            Ok(None) => {
                                session_ui.local_ended = true;
                                transfer_cancel.store(true, Ordering::Release);
                                pending_local_eof = true;
                                None
                            }
                            Err(error) => break Err(error),
                        },
                        TerminalPumpEvent::LocalInputProcessed(result) => match result {
                            Ok(Some(processed)) => {
                                if processed.action == LocalAction::CancelOp
                                    && transfer.is_some()
                                {
                                    if let Some(audit) = audit.as_ref() {
                                        cancel_action_audit = Some(Box::pin(async move {
                                    ...
                                    pending_local_input = Some(processed);
                                    None
                                }
                            }
                            Ok(None) => {
                                debug_assert!(terminal_closing.load(Ordering::Acquire));
                                None
                            }
                            Err(error) => break Err(error),
                        },
                        TerminalPumpEvent::RemoteOutputRead(result) => match result {
                            Ok(Some(chunk)) => {
                                if audit.is_some() {
                                    let step = record_remote_output_chunk(chunk, audit.as_ref());
                                    arm_reusable_step(&mut remote_output_step, step);
                                    remote_output_active = true;
                                } else if session_ui.flow.is_some() {
                                    if session_ui.delayed.append(chunk.as_slice())
                                        == AppendOutcome::Overflow
                                    {
                                        session_ui.delayed_overflow = true;
                                    }
                                } else {
                                    terminal_output.prepare_into(
                                        chunk.as_slice(),
                                        &mut display_buffer,
                                    );
                                    let display = std::mem::take(&mut display_buffer);
                                    remote_write_length = display.len() as u64;
                                    let writer = output
                                        .take()
                                        .expect("remote display writer is idle");
                                    let step = write_remote_output_owned(
                                        writer,
                                        display,
                                        None,
                                        &remote_write_phase,
                                    ...
                                    arm_reusable_step(&mut remote_write_step, step);
                                    remote_write_active = true;
                                }
                                None
                            }
                            Ok(None) => remote.observe_output_eof(tokio::time::Instant::now()),
                            Err(error) => break Err(error),
                        },
                        TerminalPumpEvent::RemoteOutputProcessed(result) => {
                        ...
                        TerminalPumpEvent::RemoteOutputWritten((writer, display, result)) => {
                            output = Some(writer);
                        ...
                        TerminalPumpEvent::RemoteExit(result) => {
                            ...
                            if let Some(audit) = audit.as_ref() {
                                ...
                                terminal_exit_audit = Some(Box::pin(async move {
                                    audit.record_terminal_exit(code).await
                                }));
                                None
                            } else {
                        ...
                        TerminalPumpEvent::RemoteExitAudited(result) => match result {
                            Ok(()) => {
                                let code = terminal_exit_code
                                    .take()
                                    .expect("the terminal-exit audit owns its exit code");
                                remote.observe_exit(code, tokio::time::Instant::now())
                            }
                            Err(error) => break Err(error.into()),
                        },
                        TerminalPumpEvent::TransferCancelAudited(result) => match result {
                            Ok(()) => {
                                transfer_cancel.store(true, Ordering::Release);
                                None
                            }
                        ...
                        TerminalPumpEvent::ResizeDetected(Some((size, resize))) => {
                            let writer = control_write
                                .take()
                                .expect("the terminal-resize writer is idle");
                            let step = forward_terminal_resize_owned(
                                writer,
                                size,
                                resize,
                                audit.as_ref(),
                            ...
                            arm_reusable_step(&mut resize_step, step);
                            resize_active = true;
                            None
                        }
                        TerminalPumpEvent::ResizeDetected(None) => None,
                        TerminalPumpEvent::ResizeProcessed(result) => match result {
                            Ok(Some(size)) => {
                        ...
                        TerminalPumpEvent::AuditPoll => {
                            ...
                            // The persistent periodic poll cannot be reset by
                            // terminal, resize or network events. A due
                            // checkpoint is handed to the pinned send branch;
                            // the substream write never suspends the pump.
                        ...
                        TerminalPumpEvent::AuditFrame(result) => {
                            ...
                            match result {
                                ...
                                Ok(None) => {
                                    audit_failure = Some(Box::pin(audit.fail_closed(
                                        None,
                        ...
                        TerminalPumpEvent::AuditSend(result) => {
                            match result {
                                Ok(()) => None,
                                Err(error) => {
                                    let audit = audit.as_ref().expect(
                                        "an audit send exists only for enterprise sessions",
                                    );
                                    audit_failure = Some(Box::pin(audit.fail_closed(
                                        None,
                                        AuditCloseReason::AuditFailure,
                        ...
                        TerminalPumpEvent::AuditFailureHandled => {
                            break Err(ControllerError::Audit(AuditError::FailedClosed));
                        }
                    };
                    if let Some(code) = completion {
                        pending_completion = Some(code);
                    }
                }
            }
        };

        // Closing polls every already-started owner together. Audit owners
        // share the observer mutex and may also need the endpoint driver, so
        // awaiting them sequentially can deadlock even though each operation
        // is individually bounded. Only a terminal step observed in its pure
        // external-I/O phase is dropped at the common absolute deadline.
        terminal_closing.store(true, Ordering::Release);
        transfer_cancel.store(true, Ordering::Release);
        let step_deadline = remote
            .deadline()
            .unwrap_or_else(|| tokio::time::Instant::now() + REMOTE_COMPLETION_TIMEOUT);
        let deadline = tokio::time::sleep_until(step_deadline);
        tokio::pin!(deadline);
        let mut deadline_active = local_input_active || remote_write_active || resize_active;
        let mut terminal_audit_cleanup: Option<PendingAuditStep<'_, ()>> = None;
        let mut audit_send_result = None;
        let mut audit_process_result = None;
        let mut closing_endpoint_error = driver.enforce_binding(binding).err();
        loop {
            if !local_input_active
                ...
                && cancel_action_audit.is_none()
                && audit_failure.is_none()
                && terminal_audit_cleanup.is_none()
                && transfer.is_none()
            {
                break;
            }
            tokio::select! {
                biased;
                result = async {
                    let (writer, result) = local_input_step
                        .as_mut()
                        .expect("the Closing local-input branch owns its slot")
                        .await;
                    local_input_active = false;
                    data_write = Some(writer);
                    result
                    ...
                    if let Err(error) = result {
                        merge_closing_error(&mut outcome, error);
                    }
                }
                result = async {
                    let result = remote_output_step
                        .as_mut()
                        .expect("the Closing remote-output branch owns its slot")
                        .await;
                    remote_output_active = false;
                    result
                }, if remote_output_active => {
                ...
                result = async {
                    let (writer, display, result) = remote_write_step
                        .as_mut()
                        .expect("the Closing display branch owns its slot")
                        .await;
                    remote_write_active = false;
                    output = Some(writer);
                    display_buffer = display;
                ...
                result = async {
                    let (writer, result) = resize_step
                        .as_mut()
                        .expect("the Closing resize branch owns its slot")
                        .await;
                    resize_active = false;
                    control_write = Some(writer);
                    result
                ...
                result = async {
                    let result = audit_send
                        .as_mut()
                        .expect("the Closing checkpoint-send branch owns its slot")
                        .await;
                    audit_send = None;
                    result
                }, if audit_send.is_some() => audit_send_result = Some(result),
                result = async {
                    let result = audit_process
                        .as_mut()
                        .expect("the Closing audit-process branch owns its slot")
                        .await;
                    audit_process = None;
                    result
                }, if audit_process.is_some() => audit_process_result = Some(result),
                result = async {
                    let result = terminal_exit_audit
                        .as_mut()
                        .expect("the Closing terminal-exit audit branch owns its slot")
                        .await;
                    terminal_exit_audit = None;
                    result
                }, if terminal_exit_audit.is_some() => {
                ...
                result = async {
                    let result = cancel_action_audit
                        .as_mut()
                        .expect("the Closing transfer-cancel audit branch owns its slot")
                        .await;
                    cancel_action_audit = None;
                    result
                }, if cancel_action_audit.is_some() => {
                ...
                () = async {
                    audit_failure
                        .as_mut()
                        .expect("the Closing audit-failure branch owns its slot")
                        .await;
                    audit_failure = None;
                }, if audit_failure.is_some() => {}
                result = async {
                    let result = terminal_audit_cleanup
                        .as_mut()
                        .expect("the Closing terminal-audit branch owns its slot")
                        .await;
                    terminal_audit_cleanup = None;
                    result
                }, if terminal_audit_cleanup.is_some() => {
                    if let Err(error) = result {
                        outcome = Err(error.into());
                    }
                }
                () = &mut deadline, if deadline_active => {
                    ...
                    let local_timed_out = local_input_active && local_input_phase.is_io();
                    let display_timed_out = remote_write_active && remote_write_phase.is_io();
                    let resize_timed_out = resize_active && resize_phase.is_io();
                    if local_timed_out {
                        local_input_step = None;
                        local_input_active = false;
                    }
                    if display_timed_out {
                        remote_write_step = None;
                        remote_write_active = false;
                    }
                    if resize_timed_out {
                        resize_step = None;
                    ...
                    if local_timed_out || display_timed_out || resize_timed_out {
                        // Enterprise finalization still needs its independent
                        // audit substream. Dropping the timed-out owner ends
                        // its external effect; the endpoint may only be closed
                        // here when no audit exchange remains.
                        if audit.is_some() {
                            deferred_terminal_close = true;
                        } else {
                            driver.close(binding.connection());
                        }
                        preserve_primary_error(
                            &mut outcome,
                            ControllerError::RemoteCompletionTimeout,
                        );
                        if let Some(audit) = audit.as_ref() {
                            terminal_audit_cleanup = Some(Box::pin(async move {
                                if local_timed_out {
                                    audit
                                        .record_send_outcome(
                                ...
                                if display_timed_out {
                                    audit
                                        .record_display_write_outcome(false, remote_write_length)
                                        .await?;
                                }
                                Ok(())
                            }));
                ...
                event = driver.next() => {
                    let affects_binding = matches!(
                        event,
                        EndpointEvent::Established { peer, .. }
                            | EndpointEvent::Closed { peer, .. }
                            if peer == binding.peer()
                    );
                    if affects_binding && closing_endpoint_error.is_none() {
                        closing_endpoint_error = driver.enforce_binding(binding).err();
                    }
                }
            }
        }
        if closing_endpoint_error.is_none() {
            closing_endpoint_error = driver.enforce_binding(binding).err();
        }
        if let Some(error) = closing_endpoint_error {
            preserve_primary_error(&mut outcome, error.into());
        }

        if let Some(Err(error)) = audit_send_result {
            let audit = audit
                .as_ref()
                .expect("an audit send exists only for enterprise sessions");
            drive_bound(
                driver,
                binding,
            ...
            .await
            .ok();
            outcome = Err(ControllerError::Audit(error));
        }
        match audit_process_result {
            None | Some(Ok(FrameEvent::None)) => {}
            Some(Ok(FrameEvent::Close(_))) => {
                let audit = audit
                    .as_ref()
                    .expect("an audit receive exists only for enterprise sessions");
                drive_bound(
                    driver,
                    binding,
                    audit.fail_closed(None, AuditCloseReason::AuditFailure),
                ...
                .await
                .ok();
                outcome = Err(ControllerError::Audit(AuditError::FailedClosed));
            }
            Some(Ok(FrameEvent::PeerAuditError(code))) => {
                let audit = audit
                    .as_ref()
                    .expect("an audit receive exists only for enterprise sessions");
                drive_bound(
    ...
    let session = if let Some(audit) = audit.as_ref()
    ...
    let output_deadline = remote
        .deadline()
        .unwrap_or_else(|| tokio::time::Instant::now() + REMOTE_COMPLETION_TIMEOUT);
    let (output_finish, output_endpoint) = settle_step_while_driving(
        driver,
        binding,
        finish_remote_output(
            &mut terminal_output,
            output.as_mut(),
            audit.as_ref(),
            output_deadline,
    ...
    let session = match output_endpoint {
        Some(error) if session.is_ok() => Err(error.into()),
        _ => session,
    };
    let session = match (session, control_write.as_mut()) {
        (Ok(code), Some(control_write)) => complete_terminal_control(
            driver,
            binding,
            &mut control_read,
            control_write,
        ...
        .await
        .map(|()| code),
        (Ok(_), None) => Err(ControllerError::RemoteCompletionTimeout),
    ...
    if deferred_terminal_close {
        // The failed terminal effect and both audit tails are now settled.
        driver.close(binding.connection());
    }
    // Raw input state is recovered before any best-effort display repair.
    // A stalled stdout consumer must never leave the user's terminal input
    // mode armed while display recovery is attempted.
    let session = finish_terminal(session, frontend.restore_raw_mode(raw_mode));
    let display_restore = if session.is_err() {
        frontend.restore_display()
    } else {
        Ok(())
    };
    finish_terminal(session, display_restore)
}

enum TerminalPumpEvent<O> {
    Cancelled,
    Driver(EndpointEvent),
    LocalInputRead(Result<Option<LocalInputChunk>, ControllerError>),
    LocalInputProcessed(Result<Option<ProcessedInput>, ControllerError>),
    RemoteOutputRead(Result<Option<TerminalChunk>, ControllerError>),
    RemoteOutputProcessed(Result<TerminalChunk, ControllerError>),
    RemoteOutputWritten((O, Vec<u8>, Result<bool, ControllerError>)),
    RemoteExit(Result<u32, ControllerError>),
    RemoteExitAudited(Result<(), AuditError>),
    TransferCancelAudited(Result<(), AuditError>),
    ResizeDetected(Option<(TerminalSize, [u8; CONTROL_LEN])>),
    ResizeProcessed(Result<Option<TerminalSize>, ControllerError>),
    Transfer(TransferOutcome),
    AuditPoll,
    AuditFrame(Result<Option<Vec<u8>>, AuditError>),
...
fn arm_reusable_step<'a, T, F>(slot: &mut Option<ReusableBoxFuture<'a, T>>, future: F)
where
    F: Future<Output = T> + Send + 'a,
{
    match slot {
        Some(slot) => slot.set(future),
        None => *slot = Some(ReusableBoxFuture::new(future)),
    }
}

async fn settle_step_while_driving<T>(
    driver: &mut EndpointDriver,
    binding: ConnectionBinding,
    step: impl Future<Output = T>,
) -> (T, Option<EndpointError>) {
    tokio::pin!(step);
    let mut endpoint_error = driver.enforce_binding(binding).err();
    loop {
        tokio::select! {
            event = driver.next() => {
                let affects_binding = matches!(
                ...
                if affects_binding && endpoint_error.is_none() {
                    endpoint_error = driver.enforce_binding(binding).err();
                }
            }
            result = &mut step => {
                ...
                return (result, endpoint_error);
            }
        }
    }
}

fn preserve_primary_error<T>(outcome: &mut Result<T, ControllerError>, cleanup: ControllerError) {
    if outcome.is_ok() {
        *outcome = Err(cleanup);
    }
}

fn merge_closing_error<T>(outcome: &mut Result<T, ControllerError>, cleanup: ControllerError) {
    if matches!(cleanup, ControllerError::Audit(_)) {
        *outcome = Err(cleanup);
    } else {
        preserve_primary_error(outcome, cleanup);
    }
}
...
fn local_input_reserve(control: &LocalControlInput) -> usize {
    usize::from(control.enabled() && control.pending_prefix())
}
...
async fn process_local_input_chunk(
    ...
    let reserve = local_input_reserve(control);
    ...
    let processed = forward_processed_input(data_write, processed, audit).await?;
    *pending_input = Some(processed);
    tokio::task::yield_now().await;
    Ok(LocalInputSignal::ChunkPending)
}

/// Owns one already-read local chunk through every ordered audit and remote
/// write effect. Active retains this future across unrelated pump events and
/// only re-arms the input read after it reaches a definite result.
#[cfg(test)]
async fn forward_processed_input(
    data_write: &mut (impl tokio::io::AsyncWrite + Unpin),
    processed: ProcessedInput,
    audit: Option<&AuditObserver>,
) -> Result<ProcessedInput, ControllerError> {
...
async fn forward_processed_input_owned<W: tokio::io::AsyncWrite + Unpin>(
    mut data_write: W,
    processed: ProcessedInput,
    audit: Option<&AuditObserver>,
    phase: &TerminalStepPhase,
    closing: &AtomicBool,
) -> (W, Result<Option<ProcessedInput>, ControllerError>) {
    if processed.remote_bytes.is_empty() {
        return (data_write, Ok(Some(processed)));
    }
    if let Some(audit) = audit
        && let Err(error) = audit.record_input(processed.remote_bytes.as_slice()).await
    {
        return (data_write, Err(error.into()));
    }
    let length = processed.remote_bytes.as_slice().len() as u64;
    ...
    .await;
    // No await may occur between the external effect completing and this
    // transition: Closing can now retain the audit-outcome owner safely.
    phase.enter_audit();
    if let Some(audit) = audit {
        let recorded = audit
            .record_send_outcome(DIRECTION_CTRL_TO_HOST, send.is_ok(), length)
            .await;
        if send.is_ok()
            && let Err(error) = recorded
        {
            return (data_write, Err(error.into()));
        }
    }
    match send {
        Ok(()) => (data_write, Ok(Some(processed))),
        Err(error) => (data_write, Err(error.into())),
    }
}
...
async fn copy_remote_output(
    ...
    loop {
        let Some(chunk) = read_remote_output_chunk(data_read).await? else {
            return Ok(());
        };
        if prompt_active.is_some() {
            let chunk = record_remote_output_chunk(chunk, audit).await?;
            ...
            write_remote_output_chunk(output, chunk, display, audit).await?;
        }
        tokio::task::yield_now().await;
    }
}

async fn read_remote_output_chunk(
    data_read: &mut (impl tokio::io::AsyncRead + Unpin),
...
async fn record_remote_output_chunk(
    chunk: TerminalChunk,
    audit: Option<&AuditObserver>,
) -> Result<TerminalChunk, ControllerError> {
    // Design section 18.3: raw network output is recorded before display
    // handling; the shared output chain always compares these raw bytes.
    if let Some(audit) = audit {
...
/// Owns one already-read remote chunk through its ordered audit, display
/// write, flush and write-outcome effects.
#[cfg(test)]
async fn write_remote_output_chunk(
    output: &mut (impl tokio::io::AsyncWrite + Unpin),
    chunk: TerminalChunk,
    display: Vec<u8>,
    audit: Option<&AuditObserver>,
) -> Result<(), ControllerError> {
    let _ = record_remote_output_chunk(chunk, audit).await?;
    if let Some(audit) = audit {
...
async fn write_remote_output_owned<O: tokio::io::AsyncWrite + Unpin>(
    mut output: O,
    display: Vec<u8>,
    audit: Option<&AuditObserver>,
    phase: &TerminalStepPhase,
    closing: &AtomicBool,
) -> (O, Vec<u8>, Result<bool, ControllerError>) {
    if let Some(audit) = audit
        && let Err(error) = audit.record_display_bytes(&display).await
    {
        return (output, display, Err(error.into()));
    }
    let length = display.len() as u64;
    if closing.load(Ordering::Acquire) {
        if let Some(audit) = audit
            && let Err(error) = audit.record_display_write_outcome(false, length).await
        {
            return (output, display, Err(error.into()));
        }
        return (output, display, Ok(false));
    }
    ...
    if let Some(audit) = audit {
        let recorded = audit
            .record_display_write_outcome(write.is_ok(), length)
            .await;
        if write.is_ok()
            && let Err(error) = recorded
        {
    ...
    match write {
        Ok(()) => (output, display, Ok(true)),
        Err(error) => (output, display, Err(error.into())),
    }
}

async fn finish_remote_output<O: tokio::io::AsyncWrite + Unpin>(
    terminal_output: &mut RemoteTerminalOutput,
    output: Option<&mut O>,
    audit: Option<&AuditObserver>,
    deadline: tokio::time::Instant,
) -> Result<(), ControllerError> {
    let tail = terminal_output.take_tail();
    if !tail.is_empty()
    ...
    let Some(output) = output else {
        ...
        {
            let _ = audit
                .record_display_write_outcome(false, tail.len() as u64)
                .await;
        }
    ...
    .await;
    let confirmed = matches!(write, Ok(Ok(())));
    let audit_outcome = if tail.is_empty() {
        Ok(())
    } else if let Some(audit) = audit {
        audit
            .record_display_write_outcome(confirmed, tail.len() as u64)
            .await
    } else {
        Ok(())
    };
    match write {
        Ok(Ok(())) => audit_outcome.map_err(ControllerError::from),
        Ok(Err(error)) => Err(ControllerError::TerminalOutput(error)),
        Err(_) => Err(ControllerError::RemoteCompletionTimeout),
    }
}
...
async fn poll_terminal_resize(
    frontend: &impl TerminalFrontend,
    poll: &mut tokio::time::Interval,
    last_size: TerminalSize,
    enabled: bool,
) -> Option<(TerminalSize, [u8; CONTROL_LEN])> {
    if !enabled {
        return std::future::pending().await;
    }
    poll.tick().await;
    changed_terminal_size(frontend, last_size).ok().flatten()
}

async fn forward_terminal_resize_owned<W: tokio::io::AsyncWrite + Unpin>(
    mut control_write: W,
    size: TerminalSize,
    resize: [u8; CONTROL_LEN],
    audit: Option<&AuditObserver>,
    phase: &TerminalStepPhase,
    closing: &AtomicBool,
) -> (W, Result<Option<TerminalSize>, ControllerError>) {
    if let Some(audit) = audit
        && let Err(error) = audit
            .record_resize(DIRECTION_CTRL_TO_HOST, size.columns(), size.rows())
            .await
    {
        return (control_write, Err(error.into()));
    }
    if closing.load(Ordering::Acquire) {
        return (control_write, Ok(None));
    }
    ...
    phase.enter_audit();
    match write {
        Ok(()) => (control_write, Ok(Some(size))),
        Err(error) => (control_write, Err(error.into())),
    }
}
...
async fn write_local_ui(
    ...
    write_local_ui_until(
        output,
        to_display,
        bytes,
        tokio::time::Instant::now() + LOCAL_OUTPUT_TIMEOUT,
    )
    .await
}

async fn write_local_ui_until(
    output: &mut (impl tokio::io::AsyncWrite + Unpin),
    to_display: bool,
    bytes: &[u8],
    deadline: tokio::time::Instant,
...
async fn flush_delayed_output(
    ...
    // write.
    let mut display = Vec::with_capacity(bytes.len() * UTF8_REPLACEMENT.len());
    terminal_output.prepare_into(&bytes, &mut display);
    if let Some(audit) = audit {
        audit.record_display_bytes(&display).await?;
    }
    let displayed = tokio::time::timeout(LOCAL_OUTPUT_TIMEOUT, async {
        terminal_output.write(output, &display).await?;
        output.flush().await
    })
    .await;
    if !matches!(displayed, Ok(Ok(()))) {
        if let Some(audit) = audit {
            let _ = audit
                .record_display_write_outcome(false, display.len() as u64)
                .await;
        ...
        return match displayed {
            ...
            Err(_) => Err(ControllerError::RemoteCompletionTimeout),
            Ok(Ok(())) => unreachable!("the successful result was handled above"),
        };
    }
    if let Some(audit) = audit {
        audit
...
async fn begin_transfer<'a>(
    ...
    if direction == TransferDirection::Upload {
        ...
        let mut source = match TokioFileTransferBackend.open_source(path.clone()).await {
            Ok(source) => source,
            Err(error) => {
                return fail_transfer_startup(direction, &error.to_string(), output, session_ui)
                    .await;
        ...
        let file_name = path
            ...
            .unwrap_or_default();
        let local_source = path.to_str().map(str::to_owned);
...
async fn poll_inflight_transfer<F>(transfer: &mut Option<Pin<Box<F>>>) -> TransferOutcome
where
    F: Future<Output = TransferOutcome> + ?Sized,
{
...
/// Moves the sole transfer owner through Closing. Cancellation prevents new
/// protocol work, while an already-started seal/commit is polled to a
/// definite outcome because Tokio cannot abort an executing blocking job.
#[cfg(test)]
async fn settle_inflight_transfer<F>(
    transfer: &mut Option<Pin<Box<F>>>,
    cancel: &AtomicBool,
...
{
    cancel.store(true, Ordering::Relaxed);
    let outcome = if transfer.is_some() {
        Some(poll_inflight_transfer(transfer).await)
    } else {
        None
    };
    drop(transfer.take());
...
async fn handle_transfer_event(
    ...
    let audit_failed = matches!(
        outcome,
        TransferOutcome::AuditFailed { .. } | TransferOutcome::AuditFailedBeforeCommit
    ) || match audit {
        Some(audit) => audit.has_failed().await,
        None => false,
    };
    let summary = transfer_summary_line_with_audit(direction, outcome, audit_failed);
    write_local_ui(output, session_ui.ui_to_display, b"\r\n").await?;
    write_local_ui(output, session_ui.ui_to_display, summary.as_bytes()).await?;
    end_transfer_modal(session_ui, output).await?;
    *transfer = None;
...
fn transfer_summary_line(direction: TransferDirection, outcome: TransferOutcome) -> String {
    transfer_summary_line_with_audit(direction, outcome, false)
}

fn transfer_summary_line_with_audit(
    direction: TransferDirection,
    outcome: TransferOutcome,
    audit_failed: bool,
) -> String {
    ...
    let mut summary = match outcome {
        ...
        TransferOutcome::CommittedUnconfirmed { bytes } => {
            format!("{verb} committed locally: {bytes} bytes; peer confirmation was not delivered")
        }
        TransferOutcome::CommitStatusUnknown { bytes } => format!(
            "{verb} commit status unknown after {bytes} bytes; verify the destination before retrying"
        ),
        TransferOutcome::AuditFailed { bytes } => {
            return format!(
                "{verb} committed: {bytes} bytes; audit recording failed; session closing"
            );
        }
        TransferOutcome::AuditFailedBeforeCommit => {
            return format!("{verb} aborted: audit recording failed; session closing");
        }
        TransferOutcome::Cancelled => format!("{verb} cancelled"),
        TransferOutcome::Failed(code) => format!("{verb} failed: {code:?}"),
    };
    if audit_failed {
        summary.push_str("; audit recording failed; session closing");
    }
    summary
}
...
mod tests {
    ...
    use crate::progress::{HEARTBEAT_INTERVAL, NoopProgress, OperationProgress};
    use crate::protocol::EnterpriseResolveUi as _;
    use crate::protocol::{
        RelayProtocolError, ResolveDeadline, ResolvedTarget, allocate_locator, resolve_peer,
        resolve_peer_for_mode,
    };
    use crate::terminal::TerminalChunk;
    ...
    use tokio::sync::{mpsc, oneshot};
    use yon_relay::{
        CallbackExternalUrl, EnterpriseAuthConfig, EnterpriseContext, ProviderCredentials,
        ProviderSecrets, RelayServeConfig, RelayServiceError, SecretFilePolicy as _,
        SystemSecretFilePolicy, run_relay_until,
    };
    use yonder_core::wire::audit::{
    ...
    use yonder_core::{
        ...
        PrivateDirectoryPolicy as _, ProtocolError, RelayResourceConfig, RetryAfter,
        SecretDocument, SystemPrivateDirectoryPolicy, TerminalSize, TerminalValue,
    };
    ...
    async fn terminal_resize_pump_sends_each_observed_size_once() {
        let (controller_audit, host_audit, _controller_dir, _host_dir) =
    ...
    async fn completion_deadline_future_is_absolute_and_optional() {
        ...
        assert!(
            tokio::time::timeout(Duration::from_millis(1), wait_for_audit_frame(None))
                .await
                .is_err()
        );
    }
    ...
    impl<W> GateWriter<W> {
        fn new(inner: W, gate: Arc<WriterGate>) -> Self {
            Self { inner, gate }
        }
    }
    ...
    #[test]
    fn terminal_pumps_make_bidirectional_progress_under_tiny_backpressure() {
        // Both test-only pump helpers retain their optional audit futures.
        // Their joined debug-build state exceeds the Windows harness's
        // default 2 MiB stack; production uses owned reusable futures on a
        // 32 MiB runtime thread. Keep this regression on the smaller 8 MiB
        // project test runtime instead of weakening either pump.
        std::thread::Builder::new()
            .name("terminal-pump-backpressure".to_owned())
            ...
            .spawn(|| {
                tokio::runtime::Builder::new_current_thread()
                    ...
                    .build()
                    .unwrap()
                    .block_on(terminal_pumps_make_bidirectional_progress_async());
            })
            .unwrap()
            .join()
            .unwrap();
    ...
    async fn terminal_pumps_make_bidirectional_progress_async() {
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn remote_output_step_keeps_its_bytes_across_an_unrelated_event() {
        let gate = WriterGate::new();
        let mut source = b"remote output".as_slice();
        let mut output = GateWriter::new(CountingWriter::default(), Arc::clone(&gate));
        let mut terminal_output = RemoteTerminalOutput::new(RemoteTerminalOutputMode::Bytes);
        let mut delayed = DelayedOutputBuffer::new(DELAYED_OUTPUT_CAP);
        let mut overflow = false;
        let prompt = None;
        {
            let step = copy_remote_output(
                &mut source,
                &mut output,
                &mut terminal_output,
                &mut delayed,
            ...
            tokio::select! {
                ...
                result = &mut step => panic!("output unexpectedly completed: {result:?}"),
            }
            gate.release();
            step.await.unwrap();
        }

        assert_eq!(output.inner.bytes, b"remote output");
        assert!(!overflow);
    }

    #[tokio::test(flavor = "current_thread")]
    async fn local_input_step_keeps_its_bytes_across_an_unrelated_event() {
        let gate = WriterGate::new();
        let mut input = b"local input".as_slice();
        let mut remote = GateWriter::new(CountingWriter::default(), Arc::clone(&gate));
        let mut control = LocalControlInput::new(false, false);
        let mut pending = None;
        {
            let step = process_local_input_chunk(
                &mut input,
                &mut remote,
                &mut control,
                &mut pending,
            ...
            tokio::select! {
                biased;
                () = gate.started.notified() => {}
                result = &mut step => panic!("input unexpectedly completed: {result:?}"),
            }
            gate.release();
            assert_eq!(step.await.unwrap(), LocalInputSignal::ChunkPending);
        }

        assert_eq!(remote.inner.bytes, b"local input");
        assert_eq!(pending.unwrap().remote_bytes.as_slice(), b"local input");
    }

    #[tokio::test(flavor = "current_thread")]
    async fn owned_terminal_steps_do_not_start_external_io_after_closing() {
        let closing = AtomicBool::new(true);

        let input_phase = TerminalStepPhase::audit();
        let processed = LocalControlInput::new(false, false).process(local_chunk(b"input"));
        let (input, result) = forward_processed_input_owned(
        ...
        assert!(!input_phase.is_io());

        let output_phase = TerminalStepPhase::audit();
        let display = b"display".to_vec();
        let (output, retained, result) = write_remote_output_owned(
            CountingWriter::default(),
            display.clone(),
            None,
            &output_phase,
        ...
        assert_eq!(retained, display);
        assert!(output.bytes.is_empty());
        assert!(!output_phase.is_io());

        let resize_phase = TerminalStepPhase::audit();
        let size = TerminalSize::new(101, 37).unwrap();
        let resize = TerminalResize::new(size).encode();
        let (control, result) = forward_terminal_resize_owned(
            CountingWriter::default(),
            size,
            resize,
            None,
        ...
        .await;
        assert!(result.unwrap().is_none());
        assert!(control.bytes.is_empty());
        assert!(!resize_phase.is_io());
    }

    #[tokio::test(flavor = "current_thread")]
    async fn owned_terminal_steps_settle_started_io_before_closing() {
        let closing = AtomicBool::new(false);

        let input_gate = WriterGate::new();
        let input_phase = TerminalStepPhase::audit();
        let processed = LocalControlInput::new(false, false).process(local_chunk(b"input"));
        let input = forward_processed_input_owned(
            GateWriter::new(CountingWriter::default(), Arc::clone(&input_gate)),
            processed,
            None,
            &input_phase,
            &closing,
        ...
        tokio::select! {
            biased;
            () = input_gate.started.notified() => {}
            _ = &mut input => panic!("input unexpectedly completed"),
        }
        assert!(input_phase.is_io());
        closing.store(true, Ordering::Release);
        input_gate.release();
        let (input, result) = input.await;
        assert_eq!(result.unwrap().unwrap().remote_bytes.as_slice(), b"input");
        assert_eq!(input.inner.bytes, b"input");
        assert!(!input_phase.is_io());

        closing.store(false, Ordering::Release);
        let output_gate = WriterGate::new();
        let output_phase = TerminalStepPhase::audit();
        let output = write_remote_output_owned(
            GateWriter::new(CountingWriter::default(), Arc::clone(&output_gate)),
            b"display".to_vec(),
            None,
            &output_phase,
            &closing,
        ...
        tokio::select! {
            biased;
            () = output_gate.started.notified() => {}
            _ = &mut output => panic!("output unexpectedly completed"),
        }
        assert!(output_phase.is_io());
        closing.store(true, Ordering::Release);
        output_gate.release();
        let (output, retained, result) = output.await;
        assert!(result.unwrap());
        assert_eq!(retained, b"display");
        assert_eq!(output.inner.bytes, b"display");
        assert!(!output_phase.is_io());

        closing.store(false, Ordering::Release);
        let resize_gate = WriterGate::new();
        let resize_phase = TerminalStepPhase::audit();
        let size = TerminalSize::new(132, 51).unwrap();
        let resize = TerminalResize::new(size).encode();
        let control = forward_terminal_resize_owned(
            GateWriter::new(CountingWriter::default(), Arc::clone(&resize_gate)),
            size,
            resize,
            None,
            &resize_phase,
        ...
        tokio::select! {
            biased;
            () = resize_gate.started.notified() => {}
            _ = &mut control => panic!("resize unexpectedly completed"),
        }
        assert!(resize_phase.is_io());
        closing.store(true, Ordering::Release);
        resize_gate.release();
        let (control, result) = control.await;
        assert_eq!(result.unwrap(), Some(size));
        assert_eq!(control.inner.bytes, resize);
        assert!(!resize_phase.is_io());
    }

    #[tokio::test(flavor = "current_thread")]
    async fn owned_terminal_steps_report_external_io_failures() {
        let closing = AtomicBool::new(false);

        let processed = LocalControlInput::new(false, false).process(local_chunk(b"input"));
        let (_, input) = forward_processed_input_owned(
            CountingWriter::failing_first_write(),
            processed,
            None,
            &TerminalStepPhase::audit(),
            &closing,
        )
        .await;
        assert!(matches!(input, Err(ControllerError::Io(_))));

        let (_, retained, output) = write_remote_output_owned(
            CountingWriter::failing_first_write(),
            b"display".to_vec(),
            None,
            &TerminalStepPhase::audit(),
        ...
        .await;
        assert_eq!(retained, b"display");
        assert!(matches!(output, Err(ControllerError::Io(_))));

        let size = TerminalSize::new(91, 29).unwrap();
        let (_, resize) = forward_terminal_resize_owned(
            CountingWriter::failing_first_write(),
            size,
            TerminalResize::new(size).encode(),
            None,
            &TerminalStepPhase::audit(),
            &closing,
        )
        .await;
        assert!(matches!(resize, Err(ControllerError::Io(_))));
    }

    #[tokio::test(flavor = "current_thread")]
    async fn owned_terminal_steps_preserve_audit_ordering_and_failure() {
        ...
        assert_eq!(input.bytes, b"input");

        let (output, _, result) = write_remote_output_owned(
            CountingWriter::default(),
            b"display".to_vec(),
            Some(controller.as_ref()),
            &TerminalStepPhase::audit(),
        ...
        .await;
        assert!(result.unwrap());
        assert_eq!(output.bytes, b"display");

        let size = TerminalSize::new(120, 42).unwrap();
        let resize = TerminalResize::new(size).encode();
        let (control, result) = forward_terminal_resize_owned(
            CountingWriter::default(),
            size,
        ...
        .await;
        assert_eq!(result.unwrap(), Some(size));
        assert_eq!(control.bytes, resize);

        let closing = AtomicBool::new(true);
        let processed = LocalControlInput::new(false, false).process(local_chunk(b"late-input"));
        let (input, result) = forward_processed_input_owned(
            CountingWriter::default(),
            processed,
            Some(controller.as_ref()),
        ...
        let (_, result) = forward_terminal_resize_owned(
            CountingWriter::default(),
            size,
            resize,
            Some(controller.as_ref()),
        ...
        let processed = LocalControlInput::new(false, false).process(local_chunk(b"rejected"));
        let (input, result) = forward_processed_input_owned(
            CountingWriter::default(),
            processed,
            Some(controller.as_ref()),
    ...
    #[test]
    fn closing_error_precedence_preserves_root_failures_but_never_hides_audit_failure() {
        let mut successful = Ok(17_u32);
        preserve_primary_error(&mut successful, ControllerError::ConnectionLost);
        assert!(matches!(successful, Err(ControllerError::ConnectionLost)));

        let mut root_failure: Result<u32, _> = Err(ControllerError::Interrupted);
        preserve_primary_error(&mut root_failure, ControllerError::ConnectionLost);
        assert!(matches!(root_failure, Err(ControllerError::Interrupted)));

        merge_closing_error(&mut root_failure, ControllerError::Timeout);
        assert!(matches!(root_failure, Err(ControllerError::Interrupted)));

        merge_closing_error(
            &mut root_failure,
            ControllerError::Audit(AuditError::FailedClosed),
        );
        assert!(matches!(root_failure, Err(ControllerError::Audit(_))));
    }

    #[tokio::test(flavor = "current_thread")]
    async fn controller_effects_that_commit_before_audit_failure_fail_closed() {
        let (controller, host, _controller_dir, _host_dir) = establish_audit_pair().await;
        let closing = AtomicBool::new(false);
        ...
        tokio::select! {
            biased;
            () = input_gate.started.notified() => {}
            _ = &mut input => panic!("input effect unexpectedly settled"),
        }
        controller
            .fail_closed(None, AuditCloseReason::AuditFailure)
            .await;
        ...
        tokio::select! {
            biased;
            () = output_gate.started.notified() => {}
            _ = &mut output => panic!("display effect unexpectedly settled"),
        }
        controller
            .fail_closed(None, AuditCloseReason::AuditFailure)
            .await;
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn remote_output_finalization_reports_missing_and_stalled_display_sinks() {
        let mut terminal_output =
            RemoteTerminalOutput::new(RemoteTerminalOutputMode::WindowsConsoleUtf8);
        assert!(terminal_output.prepare(b"\xf0").is_empty());
        assert!(matches!(
            finish_remote_output::<CountingWriter>(
                &mut terminal_output,
                None,
                None,
        ...
        assert!(terminal_output.prepare(b"\xf0").is_empty());
        let gate = WriterGate::new();
        let mut output = GateWriter::new(CountingWriter::default(), gate);
        assert!(matches!(
            finish_remote_output(
                &mut terminal_output,
                Some(&mut output),
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn windows_console_tail_is_audited_before_a_failed_write() {
        let (controller, host, controller_dir, host_dir) = establish_audit_pair().await;
        let mut terminal_output =
            RemoteTerminalOutput::new(RemoteTerminalOutputMode::WindowsConsoleUtf8);
        assert!(terminal_output.prepare(b"\xf0").is_empty());
        let mut output = CountingWriter::failing_first_write();
        ...
        assert!(matches!(
            ...
            .await,
            Err(ControllerError::TerminalOutput(_))
        ));
        ...
        let mut chunk = TerminalChunk::new();
        chunk.writable()[..6].copy_from_slice(b"remote");
        chunk.set_len(6).unwrap();
        assert!(matches!(
            write_remote_output_chunk(
                &mut CountingWriter::failing_first_write(),
                chunk,
                b"display".to_vec(),
                Some(&controller),
            )
        ...
        let record = fs::read_dir(controller_dir.path().join("audit").join("records"))
            .unwrap()
            .next()
            .unwrap()
            .unwrap()
        ...
        stream_frames(&record, &mut |record_type, payload| {
            match record_type {
                RecordType::LocalDisplayBytes => {
                    display_records.push((record_type, payload.get(40..).unwrap().to_vec()));
                }
        ...
        .unwrap();
        assert_eq!(display_records.len(), 6);
        assert_eq!(display_records[0].0, RecordType::LocalDisplayBytes);
        assert_eq!(display_records[0].1, UTF8_REPLACEMENT);
        assert_eq!(display_records[1].0, RecordType::LocalDisplayWriteOutcome);
        assert_eq!(display_records[1].1.first(), Some(&OUTCOME_UNCONFIRMED));
        assert_eq!(display_records[2].0, RecordType::LocalDisplayBytes);
        assert_eq!(display_records[2].1, UTF8_REPLACEMENT);
        assert_eq!(display_records[3].0, RecordType::LocalDisplayWriteOutcome);
        assert_eq!(display_records[3].1.first(), Some(&OUTCOME_UNCONFIRMED));
        assert_eq!(display_records[4].0, RecordType::LocalDisplayBytes);
        assert_eq!(display_records[4].1, b"display");
        assert_eq!(display_records[5].0, RecordType::LocalDisplayWriteOutcome);
        assert_eq!(display_records[5].1.first(), Some(&OUTCOME_UNCONFIRMED));
        drop(host_dir);
    }
    ...
    async fn file_transfer_modal_preflight_preserves_ui_and_substream_boundaries() {
        ...
        let mut probe_failed = TransferUi::new(true, true, true);
        let _ = probe_failed.control.process(local_chunk(b"\x1dd"));
        output.bytes.clear();
        begin_file_transfer_modal(
            TransferDirection::Download,
            &mut driver,
        ...
        assert!(
            output
                .bytes
                .windows(FILE_TRANSFER_PROBE_FAILED.len())
                .any(|window| window == FILE_TRANSFER_PROBE_FAILED.as_bytes())
        );
        assert_eq!(probe_failed.capability, CapabilityCache::Unknown);
        assert_eq!(probe_failed.control.modal_phase(), None);

        let mut open_failed = TransferUi::new(true, true, true);
        let _ = open_failed.control.process(local_chunk(b"\x1du"));
        output.bytes.clear();
        ...
        assert_eq!(open_failed.control.modal_phase(), None);
        assert!(
            output
                .bytes
                .windows(FILE_TRANSFER_OPEN_FAILED.len())
                .any(|window| window == FILE_TRANSFER_OPEN_FAILED.as_bytes())
        );
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn transfer_start_audit_failure_reports_abort_and_ends_the_session() {
        let mut session_ui = TransferUi::new(true, true, true);
        let _ = session_ui.control.process(local_chunk(b"\x1du"));
        session_ui.control.enter_transfer().unwrap();
        session_ui.direction = Some(TransferDirection::Upload);
        ...
        let error = handle_transfer_event(
            TransferOutcome::AuditFailedBeforeCommit,
            &mut output,
            &mut session_ui,
            &cancel,
        ...
        .await
        .expect_err("an authoritative audit failure must end the session");
        ...
        assert_eq!(
            output.bytes,
            b"\r\nupload aborted: audit recording failed; session closing"
        );
        assert_eq!(session_ui.control.modal_phase(), None);
        assert!(!cancel.load(Ordering::Relaxed));
    }
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn existing_audit_failure_preserves_an_uncertain_transfer_result_before_closing() {
        ...
        let error = handle_transfer_event(
            TransferOutcome::CommitStatusUnknown { bytes: 44 },
            &mut output,
            &mut session_ui,
            &cancel,
            &mut transfer,
        ...
        .await
        .expect_err("an already-failed observer must end every transfer outcome");
        ...
        assert_eq!(
            output.bytes,
            b"\r\ndownload commit status unknown after 44 bytes; verify the destination before retrying; audit recording failed; session closing"
        );
        drop(peer_audit);
    }

    #[tokio::test(flavor = "current_thread")]
    async fn session_closing_drains_the_transfer_owner_before_returning() {
        let cancel = Arc::new(AtomicBool::new(false));
        let task_cancel = Arc::clone(&cancel);
        let (started_tx, started_rx) = oneshot::channel();
        let (release_tx, release_rx) = oneshot::channel();
        let mut transfer: Option<Pin<Box<dyn Future<Output = TransferOutcome> + Send>>> =
        ...
        let settle = tokio::spawn(async move {
            let outcome = settle_inflight_transfer(&mut transfer, &task_cancel).await;
            (outcome, transfer.is_none())
        });
        started_rx.await.unwrap();
        assert!(cancel.load(Ordering::Relaxed));
        assert!(
            !settle.is_finished(),
            "Closing must retain the transfer owner"
        );
        release_tx.send(()).unwrap();

        let (outcome, released) = settle.await.unwrap();
        assert_eq!(
            outcome,
            Some(TransferOutcome::CommittedUnconfirmed { bytes: 17 })
        );
        assert!(released);

        let mut empty: Option<Pin<Box<dyn Future<Output = TransferOutcome> + Send>>> = None;
        assert_eq!(settle_inflight_transfer(&mut empty, &cancel).await, None);
    }
    ...
    fn transfer_summaries_cover_success_cancellation_and_failure() {
        ...
        assert_eq!(
            transfer_summary_line(
                TransferDirection::Download,
                TransferOutcome::CommittedUnconfirmed { bytes: 43 }
            ),
            "download committed locally: 43 bytes; peer confirmation was not delivered"
        );
        assert_eq!(
            transfer_summary_line(
                TransferDirection::Upload,
            ...
            "upload commit status unknown after 44 bytes; verify the destination before retrying"
        );
        assert_eq!(
            transfer_summary_line(
                TransferDirection::Download,
                TransferOutcome::AuditFailed { bytes: 45 }
            ),
            "download committed: 45 bytes; audit recording failed; session closing"
        );
        assert_eq!(
                    ...
                    .block_on(async {
                        ...
                        stream_frames(&record, &mut |record_type, payload| {
                            ...
                            if record_type == RecordType::LocalSendOutcome
                                && local.get(1) == Some(&OUTCOME_UNCONFIRMED)
                            {
                                failures += 1;
                            }
                            Ok(StreamAction::Continue)
    ...
    async fn enterprise_ui_prompts_and_reads_the_platform_choice() {
        ...
        assert!(prompt.contains("1) WeCom"));
        assert!(prompt.contains("2) Feishu"));
    }
    ...
    async fn enterprise_ui_rejects_an_invalid_platform_choice() {
        ...
        let selection = ui.select_provider(EnterpriseProviders::new(true, true).unwrap());
        let (chosen, prompt) = tokio::join!(selection, async {
            client_input.write_all(b"9\n2\n").await.unwrap();
            read_ui_output(client_output).await
        },);
        assert_eq!(chosen.unwrap(), EnterpriseProvider::Feishu);
        assert!(prompt.contains("Invalid selection. Enter 1 or 2."));
    }

    #[tokio::test(flavor = "current_thread")]
    async fn enterprise_ui_waits_for_progress_clear_before_prompting() {
        let (mut client_input, input) = tokio::io::duplex(16);
        let (mut client_output, output) = tokio::io::duplex(256);
        let (progress, mut commands) = mpsc::channel(4);
        let mut ui = EnterpriseControllerUi {
            input: tokio::io::BufReader::new(input),
            output,
            opener: Box::new(|_| false),
        ...
        let command = tokio::select! {
            ...
            result = &mut selection => panic!("selection completed before progress coordination: {result:?}"),
        };
        let EnterpriseUiProgress::Pause(acknowledge) = command else {
            panic!("provider prompt must first clear progress");
        };
        assert!(
            tokio::time::timeout(Duration::from_millis(20), client_output.read_u8())
                .await
                .is_err(),
            "the prompt was written before progress acknowledged the clear"
        );
        ...
        let (provider, prompt) = tokio::join!(selection, interaction);
        assert_eq!(provider.unwrap(), EnterpriseProvider::WeCom);
        assert!(prompt.contains("Select enterprise authentication provider:"));
    }

    #[tokio::test(flavor = "current_thread")]
    async fn enterprise_ui_prints_the_url_before_resuming_wait_progress() {
        let (input, _) = tokio::io::duplex(16);
        let (mut client_output, output) = tokio::io::duplex(512);
        let (progress, mut commands) = mpsc::channel(4);
        let mut ui = EnterpriseControllerUi {
            input: tokio::io::BufReader::new(input),
            output,
        ...
        let authorization = ui.open_authorization("https://relay.example.test/auth");
        tokio::pin!(authorization);

        let first = tokio::select! {
            command = commands.recv() => command.unwrap(),
            result = &mut authorization => panic!("authorization completed before clearing progress: {result:?}"),
        };
        let EnterpriseUiProgress::Pause(acknowledge) = first else {
            panic!("authorization handoff must first clear progress");
        };
        acknowledge.send(()).unwrap();

        let second = tokio::select! {
            command = commands.recv() => command.unwrap(),
            result = &mut authorization => panic!("authorization completed before wait progress: {result:?}"),
        };
        let EnterpriseUiProgress::WaitingForAuthorization(acknowledge) = second else {
            panic!("authorization handoff must resume with an explicit waiting stage");
        };
        let mut printed = vec![0_u8; 256];
        let read =
            tokio::time::timeout(Duration::from_millis(200), client_output.read(&mut printed))
                .await
                .unwrap()
                .unwrap();
        let printed = String::from_utf8_lossy(&printed[..read]);
        assert!(printed.contains("https://relay.example.test/auth"));
        acknowledge.send(()).unwrap();
    ...
    async fn enterprise_ui_authorization_prints_the_manual_url_message_only_on_opener_failure() {
        ...
        assert!(
            printed.contains("The browser could not be opened; use the URL shown above."),
            "printed: {printed}"
        );
        ...
        result.unwrap();
        assert!(printed.contains(url));
        assert!(!printed.contains("The browser could not be opened"));
    }
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn local_display_ui_has_an_absolute_write_deadline() {
        let gate = WriterGate::new();
        let mut output = GateWriter::new(CountingWriter::default(), Arc::clone(&gate));
        let deadline = tokio::time::Instant::now() + Duration::from_millis(20);
        assert!(matches!(
            super::write_local_ui_until(&mut output, true, b"blocked", deadline).await,
            Err(ControllerError::RemoteCompletionTimeout)
        ));
            ...
            .spawn(|| {
                tokio::runtime::Builder::new_current_thread()
                    ...
                    .block_on(async {
                        ...
                        .unwrap();
                        assert_eq!(outcomes, [OUTCOME_CONFIRMED, OUTCOME_UNCONFIRMED]);
                        drop(host_dir);
                    });
            })
            .unwrap()
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn enterprise_progress_driver_coordinates_prompt_wait_and_heartbeat() {
        let (commands, command_rx) = mpsc::channel(4);
        let (heartbeat, heartbeat_seen) = oneshot::channel();
        let peer = Keypair::generate_ed25519().public().to_peer_id();
        let expected = ResolvedTarget::new(peer, RelayAccessMode::Enterprise);
        let resolve = async move {
            let (pause, paused) = oneshot::channel();
            commands
                .send(EnterpriseUiProgress::Pause(pause))
                .await
                .unwrap();
            paused.await.unwrap();

            let (waiting, visible) = oneshot::channel();
            commands
                .send(EnterpriseUiProgress::WaitingForAuthorization(waiting))
                .await
                .unwrap();
            visible.await.unwrap();
            drop(commands);
            heartbeat_seen.await.unwrap();
            Ok::<_, RelayProtocolError>(expected)
        };

        let mut progress = RecordingProgress {
            authorization_heartbeat: Some(heartbeat),
            ..RecordingProgress::default()
        };
        let resolved = tokio::time::timeout(
            HEARTBEAT_INTERVAL.saturating_mul(10),
            drive_enterprise_resolve_progress(&mut progress, command_rx, resolve),
        )
        .await
        .expect("the authorization heartbeat must remain live")
        .unwrap();
        assert_eq!(resolved, expected);
        let stages = progress.stages.lock().unwrap();
        assert_eq!(stages[0], ControllerStage::ResolvingHost);
        assert!(
            stages
                .iter()
                .filter(|stage| **stage == ControllerStage::WaitingForEnterpriseAuthorization)
                .count()
                >= 2,
            "the authorization stage must be rendered immediately and on the heartbeat"
        );
    }
    ...
    const TEST_CALLBACK_CERTIFICATE_DER: &[u8] =
        include_bytes!("../tests/fixtures/localhost-test-cert.der");
    const TEST_CALLBACK_PRIVATE_KEY_DER: &[u8] =
        include_bytes!("../tests/fixtures/localhost-test-key.der");
    ...
    impl InProcessRelay {
        ...
        fn start_with_config(config: RelayServeConfig) -> Self {
    ...
    #[derive(Default)]
    struct RejectingEnterpriseUi {
        offered: Option<EnterpriseProviders>,
    }

    impl crate::protocol::EnterpriseResolveUi for RejectingEnterpriseUi {
        fn select_provider(
            &mut self,
            providers: EnterpriseProviders,
        ) -> Pin<Box<dyn Future<Output = Result<EnterpriseProvider, RelayProtocolError>> + Send + '_>>
        {
            self.offered = Some(providers);
            Box::pin(async { Err(RelayProtocolError::EnterpriseRejected) })
        }

        fn open_authorization(
            &mut self,
            _url: &str,
        ) -> Pin<Box<dyn Future<Output = Result<(), RelayProtocolError>> + Send + '_>> {
            Box::pin(async { Err(RelayProtocolError::EnterpriseRejected) })
        }
    }

    #[tokio::test(flavor = "current_thread")]
    async fn enterprise_resolve_reaches_the_real_relay_and_propagates_ui_rejection() {
        let _permit = crate::IN_PROCESS_NETWORK_GUARD.acquire().await;
        ...
        let private_directory = directory.path().join("enterprise");
        SystemPrivateDirectoryPolicy
            .protect(&private_directory)
            .unwrap();
        let secret_path = private_directory.join("wecom.secret");
        let mut secret = fs::File::create(&secret_path).unwrap();
        secret
            .write_all(b"corp_id = \"ww1234567890abcdef\"\nagent_id = 7\napp_secret = \"s3cret\"\n")
            .unwrap();
        secret.flush().unwrap();
        SystemSecretFilePolicy
            .protect_new(&secret_path, &secret)
            .unwrap();
        drop(secret);

        let providers = EnterpriseProviders::new(true, false).unwrap();
        let provider_secrets = ProviderSecrets::new(providers, Some(secret_path), None).unwrap();
        let callback_external: CallbackExternalUrl = format!("https://localhost.:{callback_port}")
            .parse()
            .unwrap();
        let enterprise = EnterpriseAuthConfig::new(
            format!("127.0.0.1:{callback_port}").parse().unwrap(),
            callback_external,
            vec![SecretDocument::new(TEST_CALLBACK_CERTIFICATE_DER.to_vec())],
            SecretDocument::new(TEST_CALLBACK_PRIVATE_KEY_DER.to_vec()),
            providers,
            provider_secrets,
        )
        .unwrap();
        let credentials = ProviderCredentials::load(&enterprise).unwrap();
        let context = EnterpriseContext::new(enterprise, credentials);
        let listen: yonder_net::RelayListenAddress =
            format!("/ip4/127.0.0.1/tcp/{relay_port}").parse().unwrap();
        let external: yonder_net::RelayExternalAddress =
            format!("/ip4/127.0.0.1/tcp/{relay_port}").parse().unwrap();
        ...
        .unwrap();
        let relay = InProcessRelay::start_with_config(config);
        ...
        assert_eq!(connected.peer(), relay_peer);
        let mut ui = RejectingEnterpriseUi::default();
        let result = resolve_peer_for_mode(
            &mut driver,
            &mut streams,
            &connected,
        ...
        assert_eq!(ui.offered, Some(providers));
        relay.stop().await;
    }
    ...
    struct RecordingProgress {
        ...
        authorization_updates: usize,
        authorization_heartbeat: Option<oneshot::Sender<()>>,
    }

    impl OperationProgress<ControllerStage> for RecordingProgress {
        fn update(&mut self, stage: ControllerStage) {
            self.stages.lock().unwrap().push(stage);
            if stage == ControllerStage::WaitingForEnterpriseAuthorization {
                self.authorization_updates += 1;
            }
            if self.authorization_updates >= 2
                && let Some(heartbeat) = self.authorization_heartbeat.take()
            {
    ...
    struct SessionFrontend {
        input: DuplexStream,
        output: DuplexStream,
        output_gate: Arc<WriterGate>,
        size: Rc<Cell<(u16, u16)>>,
        restored: Rc<Cell<bool>>,
        display_restored: Rc<Cell<bool>>,
    }

    impl TerminalFrontend for SessionFrontend {
        type Input = DuplexStream;
        type Output = GateWriter<DuplexStream>;
        type RawModeGuard = FakeRawGuard;
        ...
        fn restore_display(&self) -> Result<(), io::Error> {
            self.display_restored.set(true);
            Ok(())
        }
    ...
    enum HostEnding {
        ...
        /// Interrupt locally while the controller's display write is in flight.
        ControllerInterruptWhileDisplayBlocked,
        /// Interrupt locally while terminal-data is backpressured by the host.
        ControllerInterruptWhileInputBlocked,
    ...
    /// Returns the bytes read in this call (the caller accumulates). The
    /// owning scenario supplies the single aggregate deadline; a second
    /// per-read deadline would make slow instrumented builds fail before the
    /// authoritative scenario budget and hide its stage diagnostics.
    ...
    /// Drains output until session completion under the owning scenario's
    /// aggregate deadline.
    ...
    async fn drive_scripted_host_checkpoint(driver: &mut EndpointDriver, audit: &AuditObserver) {
        let deadline = tokio::time::Instant::now() + Duration::from_secs(10);
        let mut frames = Box::pin(wait_for_audit_frame(Some(audit)));
        let mut poll = tokio::time::interval(AUDIT_CHECKPOINT_POLL);
        poll.set_missed_tick_behavior(tokio::time::MissedTickBehavior::Skip);
        let mut local_sent = false;
        let mut local_acknowledged = false;
        let mut peer_received = false;
        loop {
            tokio::select! {
                _ = tokio::time::sleep_until(deadline) => {
                    panic!("the scripted bilateral checkpoint did not converge");
                }
                _ = driver.next() => {}
                _ = poll.tick(), if !local_sent => {
                    if audit.checkpoint_due().await {
                        drive_drained(driver, audit.send_due_checkpoint())
                            .await
                            .unwrap();
                        local_sent = true;
                    }
                    ...
                    match result {
                        Ok(Some(frame)) => {
                            match AuditMessage::decode_frame(&frame).unwrap() {
                                AuditMessage::Checkpoint(_) => peer_received = true,
                                AuditMessage::CheckpointAck(_) => local_acknowledged = true,
                                _ => {}
                            }
                            let event = drive_drained(driver, audit.handle_frame(&frame))
                                .await
                                .unwrap();
                            assert_eq!(event, FrameEvent::None);
                            frames = Box::pin(wait_for_audit_frame(Some(audit)));
                        }
                        Ok(None) => panic!("the scripted audit stream ended"),
                        Err(error) => panic!("the scripted audit stream failed: {error}"),
                    }
                }
            }
            if local_sent && local_acknowledged && peer_received {
                return;
            }
        }
    }
    ...
    async fn serve_scripted_host(
        ...
        script: &HostScript,
        ending: HostEnding,
        checkpoint_done: Option<&tokio::sync::Notify>,
        ...
        if ending == HostEnding::CheckpointThenComplete {
            let audit = audit
                .as_ref()
                .expect("the checkpoint scenario requires enterprise audit");
            drive_scripted_host_checkpoint(driver, audit).await;
            checkpoint_done
                .expect("the checkpoint scenario has a completion signal")
                .notify_one();
        }
        if let Some(audit) = audit.as_ref() {
        ...
        match ending {
            ...
            | HostEnding::ControllerInterruptWhileDisplayBlocked
            | HostEnding::ControllerInterruptWhileInputBlocked
        ...
        let mut data_open = ending != HostEnding::ControllerInterruptWhileInputBlocked;
            ...
            tokio::select! {
                ...
                result = data_read.read(&mut data_buffer), if data_open => {
                    ...
                    if read == 0 {
                        ...
                        if matches!(
                            ending,
                            HostEnding::Stall | HostEnding::ControllerInterruptWhileInputBlocked
                        ) {
                            return;
                        }
        ...
        if let Some(audit) = audit.as_ref() {
            drive_drained(driver, audit.record_terminal_exit(script.exit_code))
                .await
                .unwrap();
        }
        drive_drained(driver, async {
        ...
        if let Some(audit) = audit.as_ref() {
            drive_drained(
                driver,
                audit.close_and_finalize(
                    ManifestEnding::ShellExit(script.exit_code),
                    true,
                    ...
                    tokio::time::timeout(
                        ...
                        local.run_until(async {
                            ...
                            let frontend = SessionFrontend {
                                ...
                                output_gate: WriterGate::released(),
                                size: Rc::clone(&size),
                                restored: Rc::clone(&restored),
                                display_restored: Rc::new(Cell::new(false)),
                            };
                            let cancellation = tokio_util::sync::CancellationToken::new();
            ...
            .spawn(move || {
                ...
                runtime.block_on(async {
                    ...
                    let result = tokio::time::timeout(
                        ...
                        local.run_until(async {
                            const EXIT_CODE: u32 = 0xC000_013A;
                            ...
                            let (frontend_output, mut script_output) = tokio::io::duplex(64 * 1024);
                            let output_gate =
                                if ending == HostEnding::ControllerInterruptWhileDisplayBlocked {
                                    WriterGate::new()
                                } else {
                                    WriterGate::released()
                                };
                            let restored = Rc::new(Cell::new(false));
                            let display_restored = Rc::new(Cell::new(false));
                            let frontend = SessionFrontend {
                                input: frontend_input,
                                output: frontend_output,
                                ...
                                restored: Rc::clone(&restored),
                                display_restored: Rc::clone(&display_restored),
                            };
                            let cancellation = tokio_util::sync::CancellationToken::new();
                            ...
                            let checkpoint_done = tokio::sync::Notify::new();
                            ...
                            let controller = async {
                                ...
                                let mut progress = NoopProgress;
                                let access_mode =
                                    if ending == HostEnding::ControllerInterruptWhileInputBlocked {
                                        RelayAccessMode::Standard
                                    } else {
                                        RelayAccessMode::Enterprise
                                    };
                                let mut prepared = prepare_controller(
                                    ...
                                    ResolvedTarget::new(host, access_mode),
                                    &code,
                                    DirectUpgradePolicy::Disabled,
                                    &mut progress,
                                )
                            ...
                            let host = serve_scripted_host(
                                ...
                                &mut control_incoming,
                                &mut file_incoming,
                                (ending != HostEnding::ControllerInterruptWhileInputBlocked)
                                    .then_some(&mut audit_incoming),
                                (ending != HostEnding::ControllerInterruptWhileInputBlocked)
                                    .then_some(host_audit_root.as_path()),
                                locator,
                                &registration,
                                &target,
                                &mut host_pake,
                                ...
                                ending,
                                match ending {
                                    HostEnding::CheckpointThenComplete => Some(&checkpoint_done),
                                    _ => None,
                                },
                                None,
                            );
                            let interaction = async {
                                if ending == HostEnding::ControllerInterruptWhileDisplayBlocked {
                                    output_gate.started.notified().await;
                                    cancellation.cancel();
                                    controller_done.notified().await;
                                    drop(script_input);
                                    return drain_output_until_closed(&mut script_output).await;
                                }
                                if ending == HostEnding::ControllerDisplayFailure {
                                    drop(script_output);
                                    controller_done.notified().await;
                                ...
                                if matches!(
                                    ...
                                    // Keep the local input half open until the
                                    // audit failure has ended the controller.
                                    // Closing it here would independently
                                    // half-close terminal-data, so cross-stream
                                    // scheduling could surface terminal EOF
                                    // before the peer's audit failure.
                                    controller_done.notified().await;
                                    drop(script_input);
                                    return drain_output_until_closed(&mut script_output).await;
                                }
                                ...
                                if ending == HostEnding::ControllerInterruptWhileInputBlocked {
                                    // The host continues driving the swarm but deliberately leaves
                                    // terminal-data unread. Once this bounded write remains pending,
                                    // the controller can only be blocked in the real flow-control path.
                                    let bytes = vec![b'x'; 32 * 1024 * 1024];
                                    let mut write = Box::pin(script_input.write_all(&bytes));
                                    tokio::select! {
                                        result = &mut write => panic!(
                                            "terminal-data never applied backpressure: {result:?}"
                                        ),
                                    ...
                                    assert!(
                                        elapsed >= REMOTE_COMPLETION_TIMEOUT,
                                        "the blocked input owner escaped its shared deadline"
                                    );
                                    assert!(
                                        elapsed
                                            < REMOTE_COMPLETION_TIMEOUT + Duration::from_secs(3),
                                        "the blocked input owner outlived its shared deadline: {elapsed:?}"
                                    );
                                    drop(script_input);
                                    seen.extend(
                                        drain_output_until_closed(&mut script_output).await,
                                    );
                                    return seen;
                                }
                                if matches!(
                                    ...
                                    if ending == HostEnding::CheckpointThenComplete {
                                        checkpoint_done.notified().await;
                                    }
                            ...
                            let controller_failed = controller_result.is_err();
                            match ending {
                                HostEnding::Complete | HostEnding::CheckpointThenComplete => {
                            ...
                            assert!(restored.get(), "raw mode must be restored");
                            if controller_failed {
                                assert!(
                                    display_restored.get(),
                                    "the display repair must run after an error"
                                );
                            }
                            if matches!(
                                ending,
    ...
    #[test]
    fn in_process_controller_bounds_an_inflight_display_write() {
        run_enterprise_controller_case(HostEnding::ControllerInterruptWhileDisplayBlocked);
    }

    #[test]
    fn in_process_controller_bounds_an_inflight_input_write() {
        run_enterprise_controller_case(HostEnding::ControllerInterruptWhileInputBlocked);
    }
                ...
                runtime.block_on(async {
                    ...
                    tokio::time::timeout(
                        ...
                        local.run_until(async {
                            ...
                            assert!(
                                seen.windows(SCRIPTED_OUTPUT.len())
                                    .any(|window| window == SCRIPTED_OUTPUT),
                                "the remote terminal output must reach the frontend"
                            );
                            assert!(
                                seen.windows(TYPED_ECHO.len())
                                    .any(|window| window == TYPED_ECHO),
                                "the remote echo must reach the frontend"
                            );
