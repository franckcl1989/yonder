...
//!
//! Record types: 3.0.0 defines no non-critical record types. Every
...
pub const CONTAINER_HEADER_DOMAIN: &[u8] = b"yonder-audit-container-header-v3";
...
/// Fixed 3.0.0 record types, one per event category of the audit design.
...
mod tests {
    ...
    fn container_header_rejects_bad_magic_version_role_and_mode() {
        ...
        // Audit containers are enterprise-only in format v3. A former
    ...
    fn truncated_frames_and_headers_fail() {
        ...
        let mut unknown = test_header().encode().as_slice().to_vec();
        unknown.extend_from_slice(&encode_frame_header(2));
        unknown.extend_from_slice(&[0x1F, 0]);
        let mut reader = ContainerReader::new(&unknown).unwrap();
        assert_eq!(reader.next_frame(), Err(ProtocolError::UnknownTag(0x1F)));
