...
//! the negotiated protocol ID `/yonder/audit/2.0.0` already carries it, the
...
pub const AUDIT_PROTOCOL: &str = "/yonder/audit/2.0.0";
...
pub const AUDIT_HELLO_DOMAIN: &[u8] = b"yonder-audit-hello-v2";
pub const AUDIT_READY_DOMAIN: &[u8] = b"yonder-audit-ready-v2";
pub const CHECKPOINT_DOMAIN: &[u8] = b"yonder-audit-checkpoint-v2";
pub const CHECKPOINT_ACK_DOMAIN: &[u8] = b"yonder-audit-checkpoint-ack-v2";
pub const MANIFEST_DOMAIN: &[u8] = b"yonder-audit-manifest-v2";
pub const SEAL_DOMAIN: &[u8] = b"yonder-audit-seal-v2";
pub const LEDGER_COMMIT_DOMAIN: &[u8] = b"yonder-audit-ledger-commit-v2";
...
/// Fixed 2.0.0 audit message tags.
...
/// Fixed 2.0.0 structured audit failure codes, one per category of design
...
impl ManifestEnding {
    ...
    /// The one-byte payload of the ending variant.
    #[must_use]
    pub const fn value(self) -> u8 {
        match self {
            Self::ShellExit(code) => code,
            Self::CloseReason(reason) => reason.code(),
        }
    }

    #[must_use]
    pub const fn from_bytes(tag: u8, value: u8) -> Option<Self> {
        match tag {
...
impl JointManifest {
    ...
    pub fn encode_payload(&self) -> Result<WireBytes<MAX_AUDIT_PAYLOAD_LEN>, ProtocolError> {
        ...
        append(&mut bytes, &mut cursor, &[self.ending.value()]);
    ...
    pub fn decode_payload(bytes: &[u8]) -> Result<Self, ProtocolError> {
        ...
        let ending = ManifestEnding::from_bytes(tail[192], tail[193])
            .ok_or(ProtocolError::UnknownTag(tail[192]))?;
        let ended_normally = tail[194];
        if ended_normally > 1 {
            return Err(ProtocolError::InvalidField(ProtocolField::Reserved));
        }
        let final_checkpoint_sequence =
            u64::from_be_bytes(tail[195..203].try_into().expect("fixed eight-byte slice"));
        Ok(Self::new(
            format_version,
            SessionId::new(session_id),
...
mod tests {
    ...
    fn malformed_fixed_payloads_fail_closed() {
        ...
        let tail_start = manifest_bytes.len() - 203;
        ...
        manifest_bytes[tail_start + 193] = 0x09;
        assert_eq!(
            AuditMessage::decode_payload(AuditTag::JointManifest.code(), &manifest_bytes),
            Err(ProtocolError::UnknownTag(0x02))
        );
        // A reserved "ended normally" byte is rejected.
        manifest_bytes[tail_start + 193] = AuditCloseReason::ControllerDetach.code();
        manifest_bytes[tail_start + 194] = 0x02;
        assert_eq!(
            AuditMessage::decode_payload(AuditTag::JointManifest.code(), &manifest_bytes),
            Err(ProtocolError::InvalidField(ProtocolField::Reserved))
        );
