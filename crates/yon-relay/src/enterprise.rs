...
impl CallbackExternalUrl {
    ...
    pub fn new(url: Url) -> Result<Self, EnterpriseConfigError> {
        ...
        if !url.username().is_empty() || url.password().is_some() {
            return Err(EnterpriseConfigError::CallbackUrlUserInfo);
        }
        if url.port() == Some(0) {
            return Err(EnterpriseConfigError::CallbackUrlPortZero);
        }
...
impl FromStr for CallbackExternalUrl {
    type Err = EnterpriseConfigError;
...
pub enum EnterpriseConfigError {
    ...
    CallbackUrlScheme,
    #[error("the enterprise callback URL must not contain user information")]
    CallbackUrlUserInfo,
    #[error("the enterprise callback URL must not use port zero")]
    CallbackUrlPortZero,
...
mod tests {
    ...
    fn callback_urls_require_bare_https_origin() {
        ...
        assert!("not a URL".parse::<CallbackExternalUrl>().is_err());
        assert!(CallbackExternalUrl::new(url("https://relay.example.test/")).is_ok());
        ...
        assert!(matches!(
            CallbackExternalUrl::new(url("https://operator@relay.example.test")),
            Err(EnterpriseConfigError::CallbackUrlUserInfo)
        ));
        assert!(matches!(
            CallbackExternalUrl::new(url("https://operator:secret@relay.example.test")),
            Err(EnterpriseConfigError::CallbackUrlUserInfo)
        ));
        assert!(matches!(
            CallbackExternalUrl::new(url("https://relay.example.test:0")),
            Err(EnterpriseConfigError::CallbackUrlPortZero)
        ));
        assert!(CallbackExternalUrl::new(url("https://relay.example.test:443")).is_ok());
