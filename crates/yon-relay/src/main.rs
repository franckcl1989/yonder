...
use yon_relay::{
    CallbackExternalUrl, CallbackServer, EnterpriseAuthConfig, EnterpriseConfigError,
    EnterpriseContext, FileIdentityStore, IdentityError, IdentityStore, ProviderCredentials,
    ProviderError, ProviderSecrets, RelayServeConfig, RelayServiceError, SecretFileError,
    SecretFilePolicy, SystemSecretFilePolicy, run_relay,
};
use yonder_config::{
...
fn serve_config_with(
    ...
    let enterprise = match enterprise_config(&loaded)? {
        Some(config) => {
            CallbackServer::preflight(&config).map_err(RelayServiceError::EnterpriseCallback)?;
            let credentials =
...
"#;
    ...
    #[test]
    fn config_check_preflights_callback_tls_without_binding() {
        let directory = test_directory();
        ...
        let callback_address = occupied.local_addr().unwrap();
        let valid = format!(
            "{prefix}certificate=['callback-cert.der']\nprivate_key='callback-key.der'\ncallback_listen='{callback_address}'\ncallback_external_url='https://localhost.'\n"
        );
        write_config(directory.path(), &valid);
        ...
        execute_command(
            ...
            || serve_config_with(&loader),
            |_| panic!("config check must not enter the serving path"),
        )
        .unwrap();

        write_private_key(
            &directory.path().join("callback-key.der"),
            TEST_WSS_MISMATCHED_PRIVATE_KEY_DER,
        );
        assert!(matches!(
            serve_config_with(&loader),
            Err(AppError::Service(
                yon_relay::RelayServiceError::EnterpriseCallback(
                    yon_relay::CallbackServerError::TlsKeyMismatch
                )
            ))
        ));
        ...
        write_private_key(
            &directory.path().join("callback-key.der"),
            TEST_WSS_PRIVATE_KEY_DER,
        );
        write_config(
            directory.path(),
            &valid.replace("https://localhost.", "https://relay.example.test"),
        );
        assert!(matches!(
            serve_config_with(&loader),
            Err(AppError::Service(
                yon_relay::RelayServiceError::EnterpriseCallback(
                    yon_relay::CallbackServerError::CallbackCertificateName(_)
                )
            ))
        ));
    }
    ...
    fn enterprise_config_requires_callback_listen_url_and_tls() {
        ...
        // Malformed callback URLs fail at the real configuration boundary.
        write_config(
            directory.path(),
            &format!(
                "{prefix}certificate=['callback-cert.der']\nprivate_key='callback-key.der'\ncallback_listen='127.0.0.1:8443'\ncallback_external_url='not a URL'\n"
            ),
        );
        assert!(matches!(
            serve_config_with(&loader),
            Err(AppError::EnterpriseConfig(
                EnterpriseConfigError::CallbackUrl(_)
            ))
        ));
