...
pub trait CallbackHandler: Send + Sync {
    /// Handles a callback for one provider with the decoded code and state.
    fn handle(
        &self,
        provider: EnterpriseProvider,
        code: &str,
        state: &str,
    ) -> std::pin::Pin<Box<dyn Future<Output = CallbackResult> + Send + '_>>;
}
                ...
                accepted = listener.accept() => {
                    ...
                    tokio::spawn(async move {
                        ...
                        let service = hyper::service::service_fn(|request| {
                            handle_request(Arc::clone(&handler), request)
                        });
...
async fn handle_request(
    ...
    let result = handler.handle(provider, &code, &state).await;
...
mod tests {
    ...
    impl CallbackHandler for StubHandler {
        fn handle(
            ...
            _code: &str,
            _state: &str,
        ) -> std::pin::Pin<Box<dyn std::future::Future<Output = CallbackResult> + Send + '_>> {
            let result = self.0;
