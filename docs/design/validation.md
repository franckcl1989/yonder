...
日期：2026-07-20。当前本机环境：Windows x86_64，`rustc 1.97.0`、`cargo 1.97.1`；Linux 证据来自 Rocky Linux 8.10 x86_64 真机。本记录只陈述真实执行结果；未列为通过的门禁仍是未完成项。
...
- 真实进程 E2E 覆盖 TCP、QUIC、WS、独立 CA 的 WSS、显式信任的自签 IP SAN WSS、不可用 UDP/TCP 候选后的可用 transport、错码 OPAQUE 拒绝、真实 PTY/shell、relay 重启 Reclaim、locator Conflict 后完整换码、错误 relay PeerId 冒充和逐连接字节篡改代理。当前默认套件在 Windows 为 `9/9`；专用 `yonder_e2e_rebuild` 构建另以 `1/1` 证明 controller 销毁旧 Swarm、更换 PeerId、禁用 DCUtR、实际绑定 relay circuit 后仍完成真实终端会话。Linux 当前提交等待新 CI 原生复验，并额外执行 native PTY 的 ANSI 字节、工作目录、环境、初始/动态尺寸、Ctrl+C 与退出码验证。
...
- 生产 `Cargo.lock` 的 `449` 个依赖和 fuzz `Cargo.lock` 的 `368` 个依赖均通过 `cargo deny` 的 advisories、bans、licenses、sources；两份锁文件也通过 `cargo audit --deny warnings`，只精确忽略冻结批准且由脚本约束依赖路径/feature 的 `RUSTSEC-2026-0118`、`RUSTSEC-2026-0119`、`RUSTSEC-2024-0436`，没有新增例外。
...
- 每 target `24h` release fuzz、relay 内部对已建立 circuit 密文的延迟/截断/选择性丢弃，以及完整 Linux namespace 公网、单 NAT、端口受限或对称 NAT、IPv6、延迟、抖动和乱序矩阵仍未完成。PeerId 冒充与外部传输字节篡改已有上述真实进程证据；`10,000` 次连接/取消/resize/child-exit 单所有者乱序门禁已有本机 release 证据并已接入候选 workflow，仍等待 Linux runner 的候选复验。当前 Rocky 内核缺少 `sch_netem`，因此延迟/抖动/乱序不能在该机伪装为已验证。
...
本轮完成了配置/CLI 重构、可工作的主控/被控/relay 全主路径、真实终端、四种 relay transport、自签 IP WSS、严格 DCUtR 收敛与预提交绑定抖动的一次性 relay-only 重建、relay restart/Reclaim/Conflict、PeerId 冒充和外部篡改拒绝、容量拒绝连接绑定、上游 per-peer 限额补偿、任务与 PTY 有界关闭、秘密与文件资源边界、六目标 MSRV、五目标覆盖率、当前源码 Miri/ASan/TSan、供应链、短 fuzz、基准和 10,000 轮单所有者压力验证；前一审计快照还取得 Linux x64 musl、双 NAT TCP/有损 QUIC及 128 endpoint 资源证据。当前已推送提交 CI `24/24` 全绿；release candidate 打包与长时 fuzz、relay 内部故障、完整网络故障矩阵和固定 runner 相对回归基线仍需取得真实结果，未取得前不得创建正式 release。
