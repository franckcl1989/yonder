...
审计只对通过 Enterprise Resolve 建立的会话自动、强制、双端启用；普通 relay 会话完全禁用。企业 controller 必须在连接 relay 和打开浏览器前、企业 host 必须在连接 relay 和发布连接码前，打开并验证本地持久身份、账本和私有目录；预检失败以明确的本地错误终止，不能先制造 OAuth 或连接码副作用。企业 controller 和 host 随后仍必须在 OPAQUE 成功、唯一连接确定之后，Terminal Active、终端输入和文件传输之前重新打开存储并完成 `/yonder/audit/3.0.0` 握手，第二次打开是权限或内容在预检后变化时的权威 TOCTOU 检查。任一端不支持、身份/存储不可用或握手不一致都在 PTY 创建前失败关闭，不存在单端记录或无审计降级。
