...
yon-relay [--log-level <LEVEL>] config sources
...
- `wss_ca` 向系统信任根之外增加有界信任锚集合，仅用于 WSS；多个锚用于自签证书轮换。中继地址中的 PeerId 仍必须与完成 libp2p 身份认证后的 PeerId 一致。
- `yon-relay.toml` 必须提供 `identity`、`1..=8` 个 `listen` 和 `1..=8` 个 `external`，可提供 `wss_certificate`、`wss_private_key` 及 `[registry]`、`[resolve]`、`[circuit]` 资源覆盖。`wss_certificate` 接受单路径或 `1..=8` 个叶证书优先的有序路径，每个路径可为 DER 或只含证书块的 PEM bundle；空列表和超量列表在文件 I/O 前拒绝。Circuit Relay v2 reservation 必须携带至少一个可拨号的 relay 地址，因此程序不从 wildcard、私网或 NAT 后的 listen 地址猜测公网入口，缺失 `external` 时必须在启动边界失败。地址各不超过 `512` 字节；listen 只允许可绑定的 `/ip4`/`/ip6` transport 地址且不带 `/p2p`，并允许 wildcard/端口 `0`；external 允许 IP/DNS transport 地址且不带 `/p2p`，但拒绝未指定 IP 和端口 `0`。listen 和 external 内部都拒绝重复项，每个 external transport 必须至少有一个同类型 listen；IP 和端口允许由 NAT 改写。程序展示拨号地址时追加持久 relay PeerId。
- 配置文件最大 `64 KiB`，严格拒绝未知字段、非法 TOML、非 UTF-8、目录冒充文件、大小写归一化后重复的环境键和无效领域值。相对路径按字段来源解析：文件字段相对于该文件目录，环境字段相对于当前目录。缺失的低优先级文件可忽略，已存在但无效的高低任一层都必须失败。`wss_ca_der`、`wss_certificate_der`、`wss_private_key_der` 是 `0.1.x` 兼容键；高优先级新键覆盖低优先级旧键，同一层同时提供同组新旧键必须失败。
- 只有 WSS 需要运维侧 TLS 证书。relay 的服务端证书链和私钥必须同时提供；证书与信任锚支持 DER/PEM，私钥支持 DER/PEM 的 PKCS#1、PKCS#8 和 SEC1，PEM 文件不得混入其他块。普通 WS、TCP 和 QUIC 不使用这组证书。每个 WSS external 的 DNS 名或 IP 必须分别匹配叶证书中的 DNS SAN 或 IP SAN，`CN` 不参与匹配；任意 WSS listen 或 external 存在而未提供证书对时必须在启动边界失败。
- 自签证书和私有 CA 均受支持。单证书自签部署中，relay 使用带 `CA:FALSE`、`serverAuth` 和正确 SAN 的自签叶证书，两个 endpoint 将同一证书配置为 `wss_ca`；私有 CA 部署中，relay 发送叶证书及所需 intermediate chain，endpoint 的 `wss_ca` 指向根 CA。启动边界负责有界读取、标准类型解析、证书/私钥密码学匹配、真实 TLS 配置构造及 WSS external 的 DNS/IP SAN 匹配；有效期、用途、证书链和信任关系由真实客户端 TLS 握手最终验证，失败时关闭连接且不降级到明文。
...
wss_certificate = ["relay-leaf.pem", "relay-intermediate.pem"] # 叶证书优先
wss_private_key = "relay-key.pem" # 与证书同时提供或同时省略
...
- shell 选择直接使用 `portable-pty::CommandBuilder::new_default_prog()`，禁止第一方重复实现平台账户或命令解释器发现。Unix 由上游先使用可执行的 `SHELL`，再查询当前账户数据库中的登录 shell，并按 login-shell 语义设置 `argv[0]`；Windows 由上游使用 `ComSpec`，缺失时回退 `cmd.exe`。Windows 没有统一 API 能读取终端应用中选定的 PowerShell、CMD 或其他 profile，因此这里的“当前用户 shell”严格指操作系统命令解释器环境，而不是终端应用 profile。
...
企业部署的 `yon-relay` 可以要求发起连接的用户先证明自己仍是该企业有效成员。`[enterprise_auth]` 配置段的存在即切换模式，没有 `enabled` 开关；一个 relay 要么是普通模式、要么是企业模式，生命周期内不可切换，认证失败不降级。

企业模式只提供 Enterprise Resolve 协议。兼容性：旧 host 仍可注册；旧 connect 无法使用企业 relay；`0.1.2` 及以后的 connect 自动识别普通或企业 relay。

支持企业微信自建应用与飞书企业自建应用，每平台最多一个应用、可同时配置、用户选择其一。认证要求：当前用户属于配置企业且仍为有效内部成员；拒绝外部用户、其他企业用户、离职、停用与无法确认状态的用户。认证成功后用户身份立即销毁。

企业认证只发生在目标解析之前，不改变 host 协议、OPAQUE、terminal 或 relay 数据面。不包含企业权限系统、用户管理系统、IAM、审计平台与端到端企业授权体系；host 身份认证保持原样。
